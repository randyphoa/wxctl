...
//!   path fields against the file's directory (re-implements the CLI's
//!   `resolve_file_paths`, which is `pub(crate)` in the `wxctl` bin crate and
//!   cannot be imported — verified `crates/wxctl/src/commands/common.rs:203`).
    ...
    pub fn load(&self) -> Result<Config, String> {
        match (&self.config, &self.config_path) {
            ...
            (Some(inline), None) => {
                ...
                resolve_relative_paths(&mut config, &cwd);
                Ok(config)
            }
            (None, Some(path)) => {
                ...
                resolve_relative_paths(&mut config, dir);
                Ok(config)
            }
        }
    }
...
/// Mirror of the CLI's `resolve_file_paths` (`commands/common.rs:203`): for every
/// `PATH_FIELDS` entry matching a resource's kind, rewrite a relative path string to
/// `config_dir.join(p)`. Handles object-array items and bare-string array items.
                    ...
                    if let Some(items) = resource.data.get_mut(arr_field).and_then(|v| v.as_array_mut()) {
                        for item in items {
                            match item.as_object_mut() {
                                ...
                                None => resolve_one(item, config_dir),
                            }
                        }
                    }
                }
...
fn resolve_one(value: &mut Value, config_dir: &Path) {
    if let Some(s) = value.as_str() {
        let p = Path::new(s);
        if p.is_relative() {
            *value = Value::String(config_dir.join(p).to_string_lossy().into_owned());
        }
    }
}
