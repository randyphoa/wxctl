...
use rmcp::{
    ...
    model::{Implementation, Meta, ProgressNotificationParam, ProgressToken, ServerCapabilities, ServerInfo},
...
use crate::tools_live::{PlanOutput, ValidateOutput, shape_plan, shape_validation};
use crate::tools_mutate::{ExecuteOutput, ProgressEvent, ProgressExecutionObserver, ProgressTestObserver, TestOutput, shape_execution, shape_tests};
...
impl WxctlMcpServer {
    ...
    pub fn new(profile: &str, profile_path: Option<&str>, read_only: bool, full_trace: bool) -> Self {
        let tool_router = if read_only { Self::base_tool_router() + Self::compose_tool_router() } else { Self::base_tool_router() + Self::compose_tool_router() + Self::mutating_tool_router() + Self::compose_mutating_tool_router() };
    ...
    async fn run_mutation(&self, op: MutationOp, input: ExecuteInput, peer: Peer<RoleServer>, meta: Meta, cancel: CancellationToken) -> Result<Json<ExecuteOutput>, String> {
        ...
        match result {
            Ok(results) => {
                ...
                Ok(Json(shape_execution(&results, input.verbose, scope.run_id.clone())))
            }
            Err(e) => {
                scope.finish("failed");
...
impl WxctlMcpServer {
    ...
    #[tool(
        name = "wxctl_validate",
        description = "Validate a wxctl config. Input: exactly one of { config } (inline YAML) or { config_path } (file path), plus optional skip_post_validate:true to skip source-file-existence checks (config-tier / pre-scaffold validation). Returns { valid, errors }. Finding validation errors is a successful result (not a tool failure).",
        annotations(read_only_hint = true, destructive_hint = false)
    )]
    ...
    pub async fn wxctl_plan(&self, Parameters(input): Parameters<PlanInput>) -> Result<Json<PlanOutput>, String> {
        ...
        Ok(Json(shape_plan(&plan, input.verbose)))
    }
    ...
    pub async fn wxctl_test(&self, Parameters(input): Parameters<ConfigInput>, peer: Peer<RoleServer>, meta: Meta) -> Result<Json<TestOutput>, String> {
        ...
        match result {
            Ok(results) => {
                ...
                scope.finish(outcome);
                Ok(Json(shape_tests(&results, scope.run_id.clone())))
            }
            Err(e) => {
                scope.finish("failed");
...
impl WxctlMcpServer {
    ...
    #[tool(
        name = "compose_prompt",
        description = "Compose pipeline prompt assembly. Config mode: { input, paths?, existing_resources? } — existing_resources is a pre-rendered \"files already exist\" block injected verbatim (the caller renders it; this tool performs no FS scan). Implementation mode: { scaffold_dir, input?, config? }. Test mode: { config, test_config:true, input? }. Returns { prompt }.",
        annotations(read_only_hint = true, destructive_hint = false)
    )]
