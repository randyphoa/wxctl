...
/// Parse the two accepted MCP deployment strings into the concrete `Deployment` string the
/// engine understands: `"saas"` → `"saas"`, `"software"` → `"software-5.3.0"` (plain
/// `"software"` is not a valid concrete deployment). Any other value → an actionable error
/// naming the valid inputs. Single source of the deployment-validation message across the
/// tool surface (discovery + compose). Callers that only need the raw filter value can
/// discard the mapped result and use `?` for its validation side effect.
pub(crate) fn parse_deployment(deployment: &str) -> Result<&'static str, String> {
    match deployment {
        "saas" => Ok("saas"),
        "software" => Ok("software-5.3.0"),
        other => Err(format!("invalid deployment '{other}'. Valid values: saas, software.")),
    }
}
...
pub fn list_resource_kinds(input: &ListResourceKindsInput) -> Result<ListResourceKindsOutput, String> {
    // Validate the filter value; the raw string (`saas`/`software`) passes through to `list_kinds`.
