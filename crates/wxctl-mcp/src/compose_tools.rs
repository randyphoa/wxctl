...
use wxctl_compose::scaffold::{config_to_multidoc_yaml, scaffold_config_in_cwd};
use wxctl_compose::{PathsInput, assemble_config_prompt, assemble_implementation_prompt, assemble_test_prompt, resolve_paths, scaffold_config};
...
pub fn compose_start(input: &ComposeStartInput) -> Result<ComposeStartOutput, String> {
    match input.deployment.as_str() {
        "saas" | "software" => {}
        other => return Err(format!("invalid deployment '{other}'. Valid values: saas, software.")),
    }
...
pub fn compose_paths(input: &ComposePathsInput) -> Result<PathsOutputDto, String> {
    // Deployment::from_str accepts "saas" or "software-X.Y.Z". MCP accepts "saas"|"software"
    // and maps "software" → "software-5.3.0" since plain "software" is not a valid concrete deployment.
    let deployment = match input.deployment.as_str() {
        "saas" => "saas",
        "software" => "software-5.3.0",
        other => return Err(format!("invalid deployment '{other}'. Valid values: saas, software.")),
    };
...
pub fn compose_prompt(input: &ComposePromptInput) -> Result<PromptOutput, String> {
    ...
    if let Some(dir) = &input.scaffold_dir {
        let descriptions = match &input.config {
            Some(yaml) => descriptions_from_yaml(yaml)?,
...
fn descriptions_from_yaml(yaml: &str) -> Result<std::collections::HashMap<String, String>, String> {
...
pub fn compose_scaffold(input: &ComposeScaffoldInput) -> Result<ScaffoldOutputDto, String> {
    ...
    match &input.output_dir {
        ...
        None => {
            let (out, rewritten) = scaffold_config_in_cwd(&config, &cwd, input.dry_run);
            let (created, skipped, failed_count) = out.manifest.counts();
            let config_yaml = config_to_multidoc_yaml(&rewritten).map_err(|e| format!("serialize rewritten config: {e:#}"))?;
