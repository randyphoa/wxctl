...
pub fn validate_output(result: &ValidationResult, fix_prompt: Option<String>) -> ValidateOutput {
    ...
    ValidateOutput { valid: result.is_valid(), errors, warnings: result.advisories().to_vec(), fix_prompt }
}
...
mod tests {
    ...
    #[test]
    fn validate_output_warnings_present_only_when_nonempty() {
        use wxctl_core::ResourceSet;
        // AC9: a valid result with no advisories → warnings empty (key elided at serialize),
        // valid stays true, no fix_prompt.
        let ok = ValidationResult::success(ResourceSet::<wxctl_core::ValidatedResource>::from_sorted(vec![]));
        let out = validate_output(&ok, None);
        assert!(out.valid);
        ...
        let json = serde_json::to_value(&out).unwrap();
        assert_eq!(json["warnings"].as_array().map(|a| a.len()), Some(1), "warnings present when non-empty: {json}");
        assert!(json.get("fix_prompt").is_none(), "valid result carries no fix_prompt");
    }
}
