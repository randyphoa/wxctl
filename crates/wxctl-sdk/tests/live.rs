...
impl LiveCtx {
    ...
    pub async fn assert_plan_all_noop(&self, yaml: &str) -> anyhow::Result<()> {
        ...
        let mismatches: Vec<String> = plan.operations.iter().filter(|op| !matches!(op.operation.op_type, OperationType::NoOp)).map(|op| format!("{}/{}={:?}", op.operation.key.kind, op.operation.key.name, op.operation.op_type)).collect();
    ...
    pub async fn assert_destroyed(&self, yaml: &str) -> anyhow::Result<()> {
        self.retry("verify_destroyed", 6, Duration::from_secs(5), |_attempt| {
            ...
            async move {
                let mut config = Config::from_yaml(&yaml)?;
                let plan = self.client.plan(&mut config).await.map_err(into_anyhow)?;
                let all_gone = plan.operations.iter().all(|op| matches!(op.operation.op_type, OperationType::Create));
...
pub fn assert_plan_op_type(plan: &CompiledPlan, kind: &str, name_prefix: &str, expected: &str) {
    let op = plan.operations.iter().find(|op| &*op.operation.key.kind == kind && op.operation.key.name.starts_with(name_prefix)).unwrap_or_else(|| panic!("No operation for {kind}/{name_prefix} in plan"));
    let actual = format!("{}", op.operation.op_type);
