    ...
    LiveTest::new("test_partial_failure_validation_rejects_all")
        ...
        .run(move |ctx| async move {
            ...
            ctx.phase("verify_kb_not_created", async {
                ...
                let kb_op = plan.operations.iter().find(|op| op.operation.key.kind.as_ref() == "knowledge_base").expect("KB not in plan");
                ctx.expect("verify_kb_not_created", matches!(kb_op.operation.op_type, wxctl_engine::OperationType::Create), "Create", format!("{:?}", kb_op.operation.op_type))?;
                Ok(())
            })
            .await?;
