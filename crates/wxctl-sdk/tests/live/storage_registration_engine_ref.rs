    ...
    LiveTest::new("test_storage_registration_engine_ref_plans")
        ...
        .run(move |ctx| async move {
            ctx.phase("plan", async {
                ...
                let kinds: Vec<String> = plan.operations.iter().map(|op| op.key.kind.to_string()).collect();
                ...
                let sr = plan.operations.iter().find(|op| &*op.key.kind == "storage_registration").expect("storage_registration op");
                let eng = plan.operations.iter().find(|op| &*op.key.kind == "presto_engine").expect("presto_engine op");
                ctx.expect("plan", matches!(sr.op_type, OperationType::Create), "Create", format!("{:?}", sr.op_type))?;
                ctx.expect("plan", matches!(eng.op_type, OperationType::Create), "Create", format!("{:?}", eng.op_type))?;
                Ok(())
            })
            .await
        })
