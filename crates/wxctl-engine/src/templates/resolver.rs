...
pub struct TemplateResolver<'a> {
    ...
    max_depth: usize,
    /// Lenient mode: an unresolvable `${...}` reference (resource not in the
    /// store, or a missing field path) is kept verbatim instead of erroring.
    /// Used for reconciliation-time PARTIAL resolution on the Deferred path —
    /// refs to already-reconciled resources resolve, the rest stay templated.
    lenient: bool,
}
...
impl<'a> TemplateResolver<'a> {
    ...
    pub fn new(store: &'a RuntimeIdStore) -> Self {
        Self { store, recursion_depth: 0, max_depth: 100, lenient: false }
    }
    ...
    /// Resolve every reference the store can satisfy; keep the rest as-is.
    /// Never fails on a missing resource or field path (lenient mode) — the
    /// only errors left are structural (recursion depth), which callers may
    /// treat as "no partial resolution possible".
    pub fn resolve_partial(&mut self, template: &CompiledTemplate) -> Result<Value> {
        self.lenient = true;
        let result = self.resolve_value(&template.original);
        self.lenient = false;
        result
    }
    ...
    fn resolve_value_impl(&mut self, value: &Value) -> Result<Value> {
        match value {
            Value::String(s) if is_template(s) => {
                let resolved = self.resolve_reference(s);
                if self.lenient && resolved.is_err() {
...
mod tests {
    ...
    #[test]
    fn resolve_partial_keeps_unresolvable_templates_verbatim() {
        // One dep in the store, one not: the resolvable ref resolves, the
        // missing one stays a literal `${...}` string, literals pass through.
        let store = make_store_with(vec![("project", "p1", json!({"guid": "proj-guid-1", "name": "my-project"}))]);
        let template = compile(json!({
            "name": "train-job",
            "project_id": "${project.p1.guid}",
            "environment": "${environment.default_runtime.guid}"
        }));
        let resolved = TemplateResolver::new(&store).resolve_partial(&template).unwrap();
        assert_eq!(resolved["project_id"], json!("proj-guid-1"));
        assert_eq!(resolved["environment"], json!("${environment.default_runtime.guid}"));
        assert_eq!(resolved["name"], json!("train-job"));

        // Resource cached but the field path missing → also kept verbatim (the
        // dep exists yet carries no such field — e.g. a Computed field the
        // discovery data lacks); strict mode errors on the same input.
        let template = compile(json!({"ref": "${project.p1.missing_field}"}));
        assert_eq!(TemplateResolver::new(&store).resolve_partial(&template).unwrap()["ref"], json!("${project.p1.missing_field}"));
        assert!(TemplateResolver::new(&store).resolve(&template).is_err());
    }
