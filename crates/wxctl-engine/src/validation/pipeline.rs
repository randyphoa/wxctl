        ...
        async {
            ...
            for (idx, resource) in resources.iter_mut().enumerate() {
                ...
                for err in dep_result.errors {
                    ...
                    // to the referenced TARGET instead of the failing resource.
                    if let ValidationError::InvalidDependency { ref field_path, ref ref_kind, ref_name: ref target_name, ref allowed_kinds } = err {
...
mod tests {
    ...
    fn normalize_raw_resource_fields_branches() {
        ...
        let fields = || vec![make_field("connection_id", FieldType::String, Some(FieldReferences { resource: "orchestrate_connection".into(), field: "id".into(), also_allows: vec![], optional: false, require_ready: false }))];
