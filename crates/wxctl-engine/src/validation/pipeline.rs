...
fn effective_descriptor(base: &Arc<ResourceDescriptor>, client_factory: Option<&Arc<ClientFactory>>) -> Result<Arc<ResourceDescriptor>> {
    ...
    let effective_def = wxctl_core::schema::effective_definition(base_def, &deployment)?;
        ...
        async {
            ...
            for (idx, resource) in resources.iter_mut().enumerate() {
                ...
                if let Err(e) = validate_schema(resource, &effective.schema) {
                    wxctl_core::log_error_field!(operation_id, "validation", wxctl_core::logging::error_codes::V003, &resource.kind, resource.ref_name(), "schema", &e.to_string(), "Fix the resource schema to match the expected format");
                ...
                if !skip_post_validate && let Some(handler) = self.registry.get_handler(&resource.kind) {
                    ...
                    tracing::debug!(target: "wxctl::substage::hook", operation_id = %operation_id, hook = "post_validate", handler_kind = %resource.kind, before = %serde_json::to_string(&redact_sensitive(&data_before)).unwrap_or_default(), after = %serde_json::to_string(&redact_sensitive(&resource.data)).unwrap_or_default(), "hook payload diff");
                }
                ...
                for err in dep_result.errors {
                    if let ValidationError::InvalidDependency { ref field_path, ref ref_kind, ref ref_name, ref allowed_kinds } = err {
                        wxctl_core::log_error_field!(
                            ...
                            &ref_name,
                            field_path,
                            &format!("references '{}:{}', but schema only allows: [{}]", ref_kind, ref_name, allowed_kinds.join(", ")),
...
mod tests {
    ...
    use wxctl_core::schema::{ApiDefinition, DiscoveryDefinition, DiscoveryMethod, FieldDefinition, FieldReferences, FieldType, HookDefinition, HttpMethod, ReconciliationDefinition, ResourceDefinition, SchemaDefinition, UpdateStrategy};
    ...
    fn make_field(name: &str, field_type: FieldType, references: Option<FieldReferences>) -> FieldDefinition {
        FieldDefinition {
            ...
            required: false,
            immutable: false,
            location: wxctl_core::schema::FieldLocation::default(),
            description: None,
            validation: None,
            schema: None,
            item_type: None,
        ...
        ResourceSchema {
            resource: ResourceDefinition {
                ...
                reconciliation: ReconciliationDefinition {
                    discovery: DiscoveryDefinition { method: DiscoveryMethod::GetById, list_field: None, name_field: None, identity_match: None, absent_when: None, list_method: None, list_body: None, id_source: id_source.into() },
    ...
    fn normalize_raw_resource_fields_branches() {
        ...
        let fields = || vec![make_field("connection_id", FieldType::String, Some(FieldReferences { resource: "orchestrate_connection".into(), field: "id".into(), also_allows: vec![], optional: false }))];
    ...
    fn effective_descriptor_early_return_branches() {
        ...
        map.insert("software-5.3".to_string(), wxctl_core::schema::definition::DeploymentOverlay::default());
