...
use crate::execution::resolution::resolve_dependencies;
...
use wxctl_core::schema::DiscoveryMethod;
...
async fn enrich_and_cache(remotes: &mut [wxctl_core::RemoteResource], key: &wxctl_core::ResourceKey, registry: &ResourceRegistry, client: &wxctl_core::client::HttpClient, runtime_store: &RuntimeIdStore, operation_id: &str, is_apply: bool) -> Result<()> {
    if let Some(first_remote) = remotes.first_mut() {
        if let Some(handler) = registry.get_handler(&key.kind) {
            ...
            tracing::debug!(target: "wxctl::substage::hook", operation_id = %operation_id, hook = "post_discover", handler_kind = %key.kind, before = %serde_json::to_string(&redact_sensitive(&before)).unwrap_or_default(), after = %serde_json::to_string(&redact_sensitive(&first_remote.data)).unwrap_or_default(), "hook payload diff");
        }
    ...
    pub async fn reconcile(&self, operation_id: &str, resources: Vec<ValidatedResource>, runtime_store: &RuntimeIdStore, mode: ReconcileMode, is_apply: bool) -> Result<ReconciliationPlan> {
        ...
        async {
            ...
            for resource in resources {
                ...
                if wxctl_core::schema::is_unsupported_on(base_def, &deployment) {
                ...
                let local_resource = resolved_resource.unwrap_or_else(|| resource.clone());
                ...
                match discovery_status {
                    ...
                    DiscoveryStatus::Deferred { missing_dependencies } => {
                        ...
                        // If scoping params have unresolved template refs, discover_all skips
                        // the API call and returns empty — the resource is treated as Create.
                        let remotes = match reconciler.discover_all(operation_id, &resource, client.clone()).await {
                            Ok(remotes) => remotes,
                            Err(e) => {
                        ...
                        if remotes.is_empty() {
                            ...
                            let (decision, reason_str) =
                                if let Some(tpl) = super::schema_reconciler::identity_paths_unresolved(&resource.data, &resource.descriptor.schema) { ("CreateUnchecked", format!("unchecked: identity path has unresolved template `{tpl}`")) } else { ("Create", reason.clone()) };
