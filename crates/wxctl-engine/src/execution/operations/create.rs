    ...
    if let Some(handler) = registry.get_handler(&descriptor.name) {
        ...
        match pre_create_result {
            ...
            Ok(HookOutcome::Continue) => {
                ...
                tracing::debug!(target: "wxctl::substage::hook", operation_id = %operation_id, hook = "pre_create", handler_kind = %descriptor.name, before = %serde_json::to_string(&redact_for_log(&payload_before, &sensitive)).unwrap_or_default(), after = %serde_json::to_string(&redact_for_log(&body, &sensitive)).unwrap_or_default(), "hook payload diff");
            }
