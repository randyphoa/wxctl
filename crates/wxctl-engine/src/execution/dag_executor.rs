...
impl DagExecutor {
    ...
    pub async fn execute_with_cancel(&self, operation_id: &str, operations: Vec<Operation>, graph: &IndexGraph<ResourceKey>, cancel: CancellationToken, for_destroy: bool, runtime_ids: RuntimeIdStore) -> Result<ExecutionResults> {
        ...
        async {
            ...
            let (result_tx, mut result_rx) = mpsc::channel::<TaskResult>(self.config.parallelism * CHANNEL_BUFFER_MULTIPLIER);
            ...
            // The accumulator lives OUTSIDE the collect future so cancellation or a
            // total timeout can no longer discard partially-collected results: real
            // cloud mutations must survive into run records and the timeout message
            // instead of being replaced by an empty `cancelled` result set.
            let results_acc = Mutex::new(ExecutionResults::with_capacity(n));
            let collect_future = collect_results(&mut result_rx, n, state.clone(), semaphore.clone(), &mut join_set, &results_acc);
            ...
            // Drain results that were sent but not yet received when the collect
            // future was dropped mid-run (cancel/timeout won the select). All tasks
            // are joined at this point, so try_recv sees every send that landed.
            {
                let mut acc = results_acc.lock().unwrap();
                while let Ok(msg) = result_rx.try_recv() {
                    match msg {
                        TaskResult::Success { result, .. } => acc.succeeded.push(result),
                        TaskResult::Failure { result } => acc.failed.push(result),
                        TaskResult::Skipped { key } => acc.skipped.push(key),
                    }
                }
            }

            let mut results = results_acc.into_inner().unwrap();
    ...
    for &idx in indices_to_check {
        ...
        if state.in_degree[idx].load(Ordering::Acquire) == 0 && state.in_degree[idx].compare_exchange(0, usize::MAX, Ordering::AcqRel, Ordering::Acquire).is_ok() {
            ...
            join_set.spawn(async move {
                // A panicking task would otherwise never send its TaskResult and never
                // discharge dependents' in-degree — with the senders alive inside
                // ExecutionState the channel never closes, so collect_results waits
                // forever (and the default config has no total_timeout). Catch the
                // unwind and convert it into a normal failure so the run terminates.
                let started = Instant::now();
                if let Err(payload) = AssertUnwindSafe(execute_task(idx, state.clone(), semaphore)).catch_unwind().await {
                    handle_task_panic(idx, &state, payload, started.elapsed()).await;
                }
            });
        }
    }
...
async fn execute_task(idx: usize, state: Arc<ExecutionState>, semaphore: Arc<Semaphore>) {
    ...
    match result {
        ...
        Err(exec_result) => {
            drop(permit);
            fail_task(&state, idx, exec_result, duration).await;
        }
    }
}

/// Shared failure path for a failed (or panicked) task: mark the op failed,
/// log, notify the observer, cascade-skip dependents, and send the Failure
/// result so `collect_results` accounts for the operation.
async fn fail_task(state: &Arc<ExecutionState>, idx: usize, exec_result: ExecutionResult, duration: Duration) {
    state.failed[idx].store(true, Ordering::Release);

    let error_msg = exec_result.error.as_deref().unwrap_or("Unknown error").to_string();
    ...
    state.observer.on_task_error(&exec_result.key, &error_msg);
    state.observer.on_task_complete(&exec_result.key, false, duration, exec_result.response.as_ref());
    propagate_failure(state, idx).await;
    send_result(state, TaskResult::Failure { result: exec_result }).await;
}

/// Convert a caught task panic into the normal failure path (see the
/// catch_unwind guard in `spawn_ready_tasks`).
async fn handle_task_panic(idx: usize, state: &Arc<ExecutionState>, payload: Box<dyn std::any::Any + Send>, duration: Duration) {
    ...
    let result = ExecutionResult { key: op.key.clone(), operation: op.op_type.clone(), success: false, error: Some(format!("task panicked: {msg}")), response: None, attempts: 1 };
    fail_task(state, idx, result, duration).await;
}
...
/// Receive task results into the caller-owned `results` accumulator until all
/// `total` operations are accounted for. The accumulator lives outside this
/// future so that when the enclosing `select!` drops it on cancel/timeout, the
/// already-collected results survive.
async fn collect_results(result_rx: &mut mpsc::Receiver<TaskResult>, total: usize, state: Arc<ExecutionState>, semaphore: Arc<Semaphore>, join_set: &mut JoinSet<()>, results: &Mutex<ExecutionResults>) -> Result<()> {
    let mut processed = 0usize;

    while processed < total {
        match result_rx.recv().await {
            Some(TaskResult::Success { idx, result: exec_result }) => {
                results.lock().unwrap().succeeded.push(exec_result);
                processed += 1;
            ...
            Some(TaskResult::Failure { result: exec_result }) => {
                results.lock().unwrap().failed.push(exec_result);
                processed += 1;
            }
            Some(TaskResult::Skipped { key }) => {
                results.lock().unwrap().skipped.push(key);
                processed += 1;
            }
            // Defensive backstop only: the senders live in ExecutionState (held by
            // this future via `state`), so the channel cannot close while we loop.
            // Panicking tasks are converted to Failure sends by the catch_unwind
            // guard in spawn_ready_tasks, so every op is guaranteed a result.
            None => {
                let mut guard = results.lock().unwrap();
                let mut unaccounted = Vec::new();
                let accounted: HashSet<ResourceKey> = guard.succeeded.iter().map(|r| r.key.clone()).chain(guard.failed.iter().map(|r| r.key.clone())).chain(guard.skipped.iter().cloned()).collect();
                for idx in 0..state.operations.len() {
                    let key = &state.operations[idx].key;
                    if !accounted.contains(key) {
                        if state.failed[idx].load(Ordering::Acquire) {
                            guard.skipped.push(key.clone());
                            processed += 1;
                        } else {
