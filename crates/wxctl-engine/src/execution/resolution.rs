...
mod tests {
    ...
    fn resolve_dependencies_partial_mixed_store() {
        ...
        if let Some(f) = schema.resource.schema.fields.iter_mut().find(|f| f.name == "tools") {
            f.references = Some(wxctl_schema::schema::FieldReferences { resource: "toolkit".to_string(), field: "id".to_string(), also_allows: vec![], optional: false, require_ready: false });
        }
    ...
    fn nested_string_reference_extracted() {
        ...
        let conn_id_field = {
            ...
            f.references = Some(wxctl_schema::schema::FieldReferences { resource: "orchestrate_connection".to_string(), field: "app_id".to_string(), also_allows: vec![], optional: false, require_ready: false });
            f
        };
        let openapi_field = {
    ...
    fn map_of_references_branches() {
        ...
        let schema_with_connections = |outer: &str, inner: Option<&str>| {
            let connections_field = {
                ...
                f.references = Some(wxctl_schema::schema::FieldReferences { resource: "orchestrate_connection".to_string(), field: "connection_id".to_string(), also_allows: vec![], optional: false, require_ready: false });
                f
            };
            let inner_field = {
