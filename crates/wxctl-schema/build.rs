...
const SCHEMA_DIRS: &[(&str, &str)] = &[
    ...
    ("src/schemas/local", "schemas/local"),
...
/// Convert a field's `properties` map into a `Vec<FieldDef>`, filling each child's
/// empty `name` from its map key.
...
fn process_schemas(schemas: &[ResourceDef]) -> Vec<ProcessedResource> {
    schemas
        ...
        .map(|schema| {
            ...
            ProcessedResource { name: schema.name.clone(), required_fields, optional_fields, depends_on: deps_set.into_iter().collect(), edges }
        })
        .collect()
}
...
fn generate_rust_code(order: &[String], resources: &[ProcessedResource], include_paths: &HashMap<String, String>, schemas: &[ResourceDef]) -> String {
    ...
    for name in order {
        let const_name = mangle_name(name);
        writeln!(code, "        crate::schema::SchemaParser::parse_str({})?,", const_name).unwrap();
    }
    writeln!(code, "    ])").unwrap();
    writeln!(code, "}}").unwrap();
    writeln!(code).unwrap();
    ...
    // panics the build if a path field could be sent to the API.
    writeln!(code).unwrap();
    ...
    for schema in schemas {
        for field in &schema.schema.fields {
            if field.is_path {
                assert_eq!(field.location.as_deref(), Some("LocalOnly"), "schema '{}': field '{}' has `is_path: true` but location is {:?} (must be LocalOnly — a path must never be sent to the API)", schema.kind, field.name, field.location);
                writeln!(code, "    (\"{}\", \"{}\", None),", schema.kind, field.name).unwrap();
            }
            if let Some(props) = &field.properties {
                ...
                for key in keys {
                    let sub = &props[key];
                    if sub.is_path {
                        let sub_name = if sub.name.is_empty() { key.as_str() } else { sub.name.as_str() };
                        assert_eq!(field.location.as_deref(), Some("LocalOnly"), "schema '{}': sub-field '{}.{}' has `is_path: true` but parent array '{}' location is {:?} (must be LocalOnly)", schema.kind, field.name, sub_name, field.name, field.location);
                        writeln!(code, "    (\"{}\", \"{}\", Some(\"{}\")),", schema.kind, sub_name, field.name).unwrap();
                    }
                }
            }
        }
...
fn main() {
    ...
    for &(fs_dir, include_prefix) in SCHEMA_DIRS {
        ...
        for entry in entries {
            ...
            // Parallel-schema sibling files (e.g. `ingestion_job.software.yaml`) share
            // `resource.name` with the base file and must not enter the main loop — they
            // would trigger the duplicate-name panic. They are loaded in the second pass below.
            let is_parallel_sibling = file_name.matches('.').count() >= 2 && (file_name.ends_with(".software.yaml") || file_name.ends_with(".saas.yaml"));
            if is_yaml && !is_parallel_sibling {
