...
struct FieldReferences {
    ...
    #[serde(default)]
    optional: bool,
    /// Execution-time readiness gate (mirrors runtime `FieldReferences.require_ready`).
    /// Threaded into edge metadata only; never affects edge existence.
    #[serde(default)]
    require_ready: bool,
    /// Taxonomy marker (OQ1): `containment` marks a parent-child lifecycle edge,
    /// exported as `mechanism: containment`. Absent = ordinary reference.
    #[serde(default)]
    ...
    for field in fields {
        ...
        if let Some(ref refs) = field.references {
            ...
            edges.push(ProcessedEdge {
                ...
                target_resource: refs.resource.clone(),
                required: effectively_required && !refs.optional,
                field_required: effectively_required,
                ref_optional: refs.optional,
                union_secondary: false,
                require_ready: refs.require_ready,
                containment,
            });
