    ...
    if let Some(fields) = resource["schema"]["fields"].as_sequence() {
        ...
        if !user_fields.is_empty() {
            ...
            for field in &user_fields {
                ...
                if desc.len() > 120 {
                    // Byte slicing must land on a UTF-8 char boundary — walk back
                    // from 117 if it falls inside a multi-byte char (e.g. an em dash).
                    let mut cut = 117;
            ...
            // References section — the schema-declared linkage contract, so config
            // generation sees which fields wire to which kinds even when the prompt
            // carries no compose-paths block.
            let mut reference_lines = Vec::new();
            for field in &user_fields {
                if field["references"].as_mapping().is_none() {
                    continue;
                }
                let name = yaml_str(&field["name"]);
                let refs = &field["references"];
                let mut kinds = vec![yaml_str(&refs["resource"])];
                if let Some(also) = refs["also_allows"].as_sequence() {
                    kinds.extend(also.iter().map(yaml_str));
                }
                let templates: Vec<String> = kinds.iter().map(|k| format!("`${{{}.<ref_name>}}`", k)).collect();
                let mut notes = Vec::new();
                if yaml_str_opt(&field["type"]).as_deref() == Some("array") {
                    notes.push("each entry".to_string());
                }
                if refs["optional"].as_bool().unwrap_or(false) {
                    notes.push("a literal value for an externally managed resource is also accepted".to_string());
                } else {
                    notes.push("must resolve to a resource in this config".to_string());
                }
                reference_lines.push(format!("| `{}` | {} | {} |\n", name, templates.join(" or "), notes.join("; ")));
            }
            if !reference_lines.is_empty() {
                out.push_str("## References\n\n");
                out.push_str("These fields link to other resources in the config. Set them to the reference template shown, where `<ref_name>` is the target resource's `ref_name`; wxctl resolves the value and orders creation automatically. A plain literal string creates no dependency edge.\n\n");
                out.push_str("| Field | Set to | Notes |\n");
                out.push_str("|-------|--------|-------|\n");
...
mod tests {
    ...
    /// Kinds with schema-declared `references:` render a References section, so the
    /// linkage contract reaches the generation prompt even without a paths block.
    #[test]
    fn renders_references_section_for_linked_kinds() {
        let only: HashSet<&str> = ["agent"].into_iter().collect();
        let md = render_kinds_markdown(Some(&only)).expect("renders");
        assert!(md.contains("## References"), "agent declares references");
        assert!(md.contains("`${tool.<ref_name>}`"), "tools field links to tool");
        assert!(md.contains("each entry"), "array reference fields note per-entry templates");

        // also_allows alternates render as `or` alternatives.
        let only: HashSet<&str> = ["model_tracking"].into_iter().collect();
        let md = render_kinds_markdown(Some(&only)).expect("renders");
        assert!(md.contains("`${wml_model.<ref_name>}` or `${wml_function.<ref_name>}`"), "also_allows kinds render as alternatives");

        // No schema-declared references → no section.
        let only: HashSet<&str> = ["project"].into_iter().collect();
        let md = render_kinds_markdown(Some(&only)).expect("renders");
        assert!(!md.contains("## References"), "project has no references");
    }
