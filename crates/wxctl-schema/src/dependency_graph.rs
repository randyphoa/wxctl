...
//! - `IndexGraph<ResourceKey>` (via `wxctl_core::IndexGraph`): Runtime resource dependencies
...
#[inline]
fn get_edge_tuples_by_index(idx: usize) -> &'static [EdgeTuple] {
    RESOURCE_EDGES[idx]
}

/// Returns dependency edge tuples for a resource by name. Each tuple is the
/// full `EdgeTuple` taxonomy record (see its doc for field order).
#[inline]
pub fn get_edges(name: &str) -> Option<&'static [EdgeTuple]> {
...
/// A one-sided bridge opportunity: exactly one endpoint kind of a bridge is
/// present in a config while the counterpart kind is absent. Pure data returned
/// by `orphan_bridge_opportunities`; the caller anchors it to a specific orphan
/// resource and renders the V505 advisory.
#[derive(Debug, Clone, PartialEq)]
pub struct BridgeOpportunity {
    pub bridge_name: &'static str,
    pub source_kind: &'static str,
    pub target_kind: &'static str,
    /// The endpoint kind present in the config (one of source_kind/target_kind).
    pub present_kind: &'static str,
    /// The endpoint kind absent from the config (the other of source/target).
    pub missing_kind: &'static str,
    /// Bridge field mappings as `(source_field, target_field)`.
    pub field_mappings: Vec<(&'static str, &'static str)>,
}

/// A representative concrete `software` deployment. Today's constraints are
/// flavor-level, so any concrete software version matches at flavor granularity.
fn software_representative() -> Deployment {
    Deployment::from_str("software-5.3.0").expect("valid representative deployment")
}

/// Compute `start_kind`'s transitive required-only dependency closure, minus
/// `present_kinds` and `start_kind` itself, attributing each still-absent kind to
/// the required in-closure edge that pulls it in. Completes an unresolved-reference
/// suggestion with the chain of resources that adding `start_kind` would also need.
///
/// Semantics mirror `compute_closure_from_kinds_with_config` (required edges only:
/// `field.required && !reference.optional`; ancestors required; bridge expansion for
/// explicitly-requested endpoints). Known limit: `LocalOnly`/`Computed` reference
/// fields produce no graph edge (see
/// docs/troubleshoot/ordering-only-reference-fields-localonly-no-edge-fix.md), so a
/// chain routed only through such a field under-reports. Acceptable.
///
/// Returns `(missing_kind, required_by_kind, field)` triples, sorted for
/// determinism. An in-closure kind with no required in-closure edge (e.g. pulled
/// by a bridge) is attributed to `start_kind` with an empty field. No I/O; wasm-safe.
pub fn missing_required_closure(start_kind: &str, present_kinds: &[&str], deployment: &Deployment) -> Vec<(String, String, String)> {
    if resource_index(start_kind).is_none() {
        return Vec::new();
    }
    let closure = SchemaDependencyGraph::compute_closure_from_kinds_with_config(&[start_kind], None, deployment);
    let present: HashSet<&str> = present_kinds.iter().copied().collect();

    let mut out: Vec<(String, String, String)> = Vec::new();
    for kind in closure.iter_order() {
        if kind == start_kind || present.contains(kind) {
            continue;
        }
        let Some(kind_idx) = resource_index(kind) else { continue };
        let mut attributed = false;
        for src in closure.iter_order() {
            if src == kind {
                continue;
            }
            let Some(src_idx) = resource_index(src) else { continue };
            for &(field, tidx, required, ..) in get_edge_tuples_by_index(src_idx) {
                if tidx == kind_idx && required {
                    out.push((kind.to_string(), src.to_string(), field.to_string()));
                    attributed = true;
                    break;
                }
            }
        ...
        if !attributed {
            out.push((kind.to_string(), start_kind.to_string(), String::new()));
        }
    }
    out.sort();
    out
...
/// Find one-sided bridge opportunities for a set of present kinds. For each bridge
/// active under the effective deployment mode with exactly one endpoint kind present
/// (counterpart absent), returns a `BridgeOpportunity`.
///
/// `deployment` selects the activation mode:
/// - `Some(d)`: a bridge counts when `bridge_active_for(bridge, d)`.
/// - `None` (conservative): a bridge counts only when active for BOTH the saas and a
///   representative software deployment, i.e. deployment-unconditional with endpoints
///   supported on every deployment flavor.
///
/// No I/O; wasm-safe. The caller decides orphan-ness and renders the advisory.
pub fn orphan_bridge_opportunities(present_kinds: &[&str], deployment: Option<&Deployment>) -> Vec<BridgeOpportunity> {
    let present: HashSet<&str> = present_kinds.iter().copied().collect();
    let software = software_representative();
    let mut out = Vec::new();
    for bridge in BRIDGES {
        let active = match deployment {
            Some(d) => bridge_active_for(bridge, d),
            None => bridge_active_for(bridge, &Deployment::Saas) && bridge_active_for(bridge, &software),
        };
        if !active {
            continue;
        }
        let &(bridge_name, source_idx, target_idx, _constraints, field_mappings, _when) = bridge;
        let source_kind = RESOURCES[source_idx].0;
        let target_kind = RESOURCES[target_idx].0;
        let (present_kind, missing_kind) = match (present.contains(source_kind), present.contains(target_kind)) {
            (true, false) => (source_kind, target_kind),
            (false, true) => (target_kind, source_kind),
            _ => continue,
        };
        out.push(BridgeOpportunity { bridge_name, source_kind, target_kind, present_kind, missing_kind, field_mappings: field_mappings.to_vec() });
    }
    out
}
...
/// use `wxctl_core::IndexGraph<ResourceKey>` for runtime resource instance dependencies.
#[derive(Debug)]
...
impl SchemaDependencyGraph {
    ...
    pub fn activated_edges(&self, name: &str) -> Vec<EdgeInfo> {
        ...
        get_edge_tuples_by_index(idx)
            .iter()
            .filter(|&&(_, target_idx, ..)| self.included_set.contains(&target_idx))
            .map(|&(field_name, target_index, required, field_required, ref_optional, union_secondary, require_ready, _containment)| EdgeInfo { field_name, target_index, required, field_required, ref_optional, union_secondary, require_ready })
            .collect()
    }
...
mod tests {
    ...
    fn test_enumerate_paths_collapses_multivalue_to_one_of() {
        ...
        assert!(dst.values.contains(&"postgresql") && dst.values.contains(&"netezza"), "one_of should list all datasource values, got {:?}", dst.values);
    ...
    #[test]
    fn missing_required_closure_wml_model_chain() {
        // AC5: wml_model's transitive required closure (minus present kinds) is
        // autoai_experiment (via wml_model.experiment) then data_asset (via
        // autoai_experiment.training_data). Bound to the REAL compiled graph.
        let chain = missing_required_closure("wml_model", &[], &Deployment::Saas);
        assert!(chain.contains(&("autoai_experiment".to_string(), "wml_model".to_string(), "experiment".to_string())), "missing autoai_experiment edge: {chain:?}");
        assert!(chain.contains(&("data_asset".to_string(), "autoai_experiment".to_string(), "training_data".to_string())), "missing data_asset edge: {chain:?}");
        assert_eq!(chain.len(), 2, "closure changed; re-pin against the compiled graph: {chain:?}");
        // A present kind is subtracted from the closure.
        let with_present = missing_required_closure("wml_model", &["autoai_experiment"], &Deployment::Saas);
        assert!(!with_present.iter().any(|(k, _, _)| k == "autoai_experiment"), "present kind must be excluded: {with_present:?}");
    }

    #[test]
    fn orphan_bridge_opportunities_common_core_connection() {
        // AC6: a lone common_core_connection has two one-sided bridges to
        // orchestrate_connection (database_access + object_storage_access).
        let opps = orphan_bridge_opportunities(&["common_core_connection"], None);
        assert_eq!(opps.len(), 2, "expected two bridge opportunities, got {:?}", opps.iter().map(|o| o.bridge_name).collect::<Vec<_>>());
        for o in &opps {
            assert_eq!(o.present_kind, "common_core_connection");
            assert_eq!(o.missing_kind, "orchestrate_connection");
        }
        let names: Vec<&str> = opps.iter().map(|o| o.bridge_name).collect();
        assert!(names.contains(&"database_access") && names.contains(&"object_storage_access"), "bridges: {names:?}");
        // AC7 (helper level): both endpoints present → no one-sided opportunity.
        let both = orphan_bridge_opportunities(&["common_core_connection", "orchestrate_connection"], None);
        assert!(both.is_empty(), "both endpoints present must yield no opportunity: {both:?}");
    }

    #[test]
    fn orphan_bridge_opportunities_thread_deployment() {
        // AC8 (unit): the deployment argument is threaded. No shipping bridge is
        // deployment-gated today, so the conservative default (None) and an explicit
        // saas both yield the always-active ccc bridges, and software adds none beyond saas.
        let conservative = orphan_bridge_opportunities(&["common_core_connection"], None).len();
        let saas = orphan_bridge_opportunities(&["common_core_connection"], Some(&Deployment::Saas)).len();
        let sw = orphan_bridge_opportunities(&["common_core_connection"], Some(&Deployment::from_str("software-5.3.0").unwrap())).len();
        assert_eq!(conservative, saas, "conservative default must match saas for always-active bridges");
        assert!(sw <= saas, "software must not activate more bridges than saas");
        assert_eq!(conservative, 2);
    }
}
