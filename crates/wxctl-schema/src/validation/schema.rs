...
fn apply_defaults_nested(value: &mut Value, schema: &SchemaDefinition) {
    match value {
        Value::Object(obj) => {
        ...
        for field in &active_fields {
            if field.required && resource.data.get(&field.name).is_none() {
                if is_common_field(&def.schema, &field.name) {
                    return Err(ValidationError::MissingField { field: field.name.clone(), reference_kind: field.references.as_ref().map(|r| r.resource.clone()) });
                }
    ...
    for field in &active_fields {
        if let Some(value) = resource.data.get(&field.name) {
            ...
            if let Err(e) = validate_field_type(value, &field.field_type, &field.name) {
                ...
                return Err(ValidationError::InvalidFieldValue { field: field.name.clone(), message });
            }
...
fn validate_nested_value(value: &Value, field: &FieldDefinition, path: &str, resource_kind: &str, resource_name: &str) -> Result<(), ValidationError> {
...
fn validate_nested_object(value: &Value, schema: &SchemaDefinition, path: &str, resource_kind: &str, resource_name: &str) -> Result<(), ValidationError> {
    ...
    for field in &schema.fields {
        ...
        match obj.get(&field.name) {
            None => {
                if field.required {
                    return Err(ValidationError::MissingField { field: field_path, reference_kind: field.references.as_ref().map(|r| r.resource.clone()) });
                }
            }
            Some(v) => {
...
mod tests {
    ...
    fn make_nested_schema() -> ResourceSchema {
        let mut target_type = make_field("target_type", FieldType::String, true);
        target_type.allowed_values = Some(vec!["subscription".into(), "instance".into()]);
        let mut count = make_field("count", FieldType::Integer, false);
        count.default = Some(json!(1));
        let mut target = make_field("target", FieldType::Object, false);
        target.schema = Some(Box::new(SchemaDefinition { fields: vec![target_type, count], ..Default::default() }));

        let value = make_field("value", FieldType::Float, false);
        let mut thresholds = make_field("thresholds", FieldType::Array, false);
        thresholds.item_type = Some(Box::new(FieldType::Object));
        thresholds.schema = Some(Box::new(SchemaDefinition { fields: vec![value], ..Default::default() }));

        let mut tags = make_field("tags", FieldType::Array, false);
        tags.item_type = Some(Box::new(FieldType::String));

        make_schema(vec![target, thresholds, tags])
    }
    ...
    fn array_element_allowed_values_enforced() {
        let mut environment = make_field("environment", FieldType::Array, false);
        environment.item_type = Some(Box::new(FieldType::String));
        environment.allowed_values = Some(vec!["draft".into(), "live".into()]);
        let schema = make_schema(vec![environment]);
    ...
    fn validate_schema_type_mismatch_rejects() {
        // Each row: (field_type, value, expected type named in the message). The
        ...
        let cases: &[(FieldType, Value, &str)] = &[
            ...
            (FieldType::Array, json!("draft"), "array"),                         // scalar where array wanted (environment-field regression)
        ];
