    ...
    extract_references_with_path(data, "", &mut |ref_str, field_path| {
        if let Some(dep_key) = parse_reference(ref_str) {
            ...
            if !available_keys.contains(&dep_key) {
                // Known resource kind but no matching resource in this config →
                // provably unresolvable at execution time (the TemplateResolver
                // reads only ids the config's own DAG produced). Hard error.
                // An UNKNOWN kind keeps today's silent skip: it may be an
                // externally-managed referent or a non-resource token.
                if crate::dependency_graph::resource_index(dep_key.kind.as_ref()).is_some() {
                    let present_kinds: Vec<&str> = available_resources.iter().map(|(_, kind)| kind.as_str()).collect();
                    let required_chain = crate::dependency_graph::missing_required_closure(dep_key.kind.as_ref(), &present_kinds, &crate::deployment::Deployment::Saas);
                    errors.push(ValidationError::UnresolvedReference { field_path: field_path.to_string(), ref_kind: dep_key.kind.to_string(), ref_name: dep_key.name.to_string(), required_chain });
                }
                return;
            }
...
mod tests {
    ...
    fn dangling_ref_known_kind_errors_v005() {
        ...
        match &result.errors[0] {
            ValidationError::UnresolvedReference { ref_kind, ref_name, .. } => {
                assert_eq!(ref_kind, "tool");
                assert_eq!(ref_name, "missing_tool");
            }
            other => panic!("expected UnresolvedReference, got {other:?}"),
        }
        // Dual-exit suggestion carries both add-resource and literal-alternative elements.
        let s = result.errors[0].suggestion();
        assert!(s.contains("`tool` resource with `ref_name: missing_tool`"), "suggestion missing add-resource element: {s}");
        assert!(s.contains("replace the reference with a literal value if the resource is managed outside it"), "suggestion missing literal alternative: {s}");
        // Message carries the V005 code.
        assert!(result.errors[0].to_string().contains("WXCTL-V005"));
    }
    ...
    #[test]
    fn unknown_kind_template_untouched_no_new_error() {
        // AC3: a `${notakind.y}` whose kind is NOT a known resource kind keeps today's
