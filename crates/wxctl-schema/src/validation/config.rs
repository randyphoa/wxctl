...
/// Map every kind to its base `ResourceDescriptor`, built once from the compiled
/// schema set. (Offline: no deployment overlay applied.)
fn descriptor_map() -> anyhow::Result<HashMap<String, Arc<ResourceDescriptor>>> {
    let mut map = HashMap::new();
    for schema in crate::load_all_schemas()? {
