...
impl<N: Clone + Eq + Hash + std::fmt::Debug> IndexGraph<N> {
    ...
    ///
    /// # Panics
    /// Panics if `idx >= self.len()`. Use [`IndexGraph::get_node`] for a
    ...
    #[inline]
    pub(crate) fn add_edge_by_idx(&mut self, from_idx: usize, to_idx: usize) {
        debug_assert!(to_idx < self.nodes.len(), "add_edge_by_idx: to_idx {} out of bounds (len {})", to_idx, self.nodes.len());
        if !self.dependencies[from_idx].contains(&to_idx) {
            self.dependencies[from_idx].push(to_idx);
        }
    }
    ...
    #[must_use]
    pub(crate) fn find_cycle_indices(&self) -> Option<Vec<usize>> {
        let n = self.len();
        if n == 0 {
            return None;
        }
