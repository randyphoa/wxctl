...
pub fn parse_spec(spec: &Value, tools_filter: Option<&[String]>, connection_id: Option<&str>, default_permission: Option<&str>) -> Result<Vec<ParsedTool>> {
    ...
    for (path, path_item) in paths {
        ...
        for method in &methods {
            ...
            if !filter_all {
                match tools_filter.unwrap().iter().find(|f| *f == operation_id) {
                    Some(matched) => {
                        matched_filters.insert(matched.as_str());
                    }
                    None => continue,
                }
            }
    ...
    // A `tools:` filter entry that matched no operationId is a config typo — fail
    // loudly at validate/plan time instead of silently generating fewer tools.
    if !filter_all {
        let unmatched: Vec<&str> = tools_filter.unwrap().iter().map(|s| s.as_str()).filter(|f| !matched_filters.contains(f)).collect();
        if !unmatched.is_empty() {
            anyhow::bail!("tools filter entries matched no operationId in the OpenAPI spec: {}", unmatched.join(", "));
        }
    }
