...
pub async fn wait_for_ready<'a>(_resource: &'a Value, response: &'a mut Value, _client: &'a HttpClient, _operation_id: &'a str) -> Result<()> {
    CatalogHandler.post_create(_resource, response, _client, _operation_id).await
}
