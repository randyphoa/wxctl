    ...
    fn pre_delete<'a>(&'a self, resource: &'a Value, _fields: &'a [FieldDescriptor], client: &'a HttpClient, _endpoint: &'a str, operation_id: &'a str) -> Pin<Box<dyn Future<Output = Result<HookOutcome>> + Send + 'a>> {
        Box::pin(async move {
            ...
            let delete_futures = delete_endpoints.iter().map(|ep| {
                ...
                let ep = ep.clone();
                async move {
                    // `not_found_ok()` suppresses the spurious error event; a 404 means the
                    // version is already gone (e.g. the sibling delete cascaded), so map it
                    // to a no-op so `join_all_ok` does not abort the whole destroy.
                    let spec = RequestSpec::new(Method::DELETE, &ep).body(BodyKind::None).not_found_ok();
                    match client.execute::<Value>(&op_id, spec).await {
                        Ok(v) => Ok(v),
                        Err(e) if wxctl_core::client::error_has_status(&e, 404) => Ok(Value::Null),
                        Err(e) => Err(e),
                    }
                }
            });
...
impl BusinessTermsHandler {
    ...
    async fn fetch_existing_terms<'a>(&'a self, client: &'a HttpClient, operation_id: &'a str) -> Result<HashMap<String, ExistingTerm>> {
        ...
        let all_published = crate::util::fetch_all_pages(client, operation_id, published_endpoint, "resources").await?;
        ...
        let all_drafts = crate::util::fetch_all_pages(client, operation_id, draft_endpoint, "resources").await?;
...
fn version_delete_endpoints(artifact_id: &str, draft_version_id: Option<&str>, published_version_id: Option<&str>) -> Vec<String> {
    ...
    // A never-published draft stores the same version_id in BOTH slots (see the
    // standalone-draft branch of fetch_existing_terms) — emitting the endpoint twice
    // makes the losing parallel DELETE 404. Skip the published slot when it
    // duplicates the draft's version_id.
    if let Some(vid) = published_version_id.filter(|v| !v.is_empty() && Some(*v) != draft_version_id) {
        out.push(format!("/v3/glossary_terms/{}/versions/{}?skip_workflow_if_possible=true", artifact_id, vid));
    }
    out
}
...
mod tests {
    ...
    fn version_delete_endpoints_cases() {
        ...
        let cases: &[Case] = &[
            ...
            // Never-published draft: both slots carry the same version_id → one delete, not two.
            ("dedupes published slot equal to draft", Some("v-1"), Some("v-1"), vec!["/v3/glossary_terms/art-1/versions/v-1?skip_workflow_if_possible=true"]),
        ];
