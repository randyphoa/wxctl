...
impl ResourceHandler for ScriptAssetHandler {
    ...
    fn pre_create<'a>(&'a self, resource: &'a mut Value, _fields: &'a [FieldDescriptor], client: &'a HttpClient, _endpoint: &'a str, operation_id: &'a str) -> Pin<Box<dyn Future<Output = Result<HookOutcome>> + Send + 'a>> {
        Box::pin(async move {
            let name = resource.get("name").and_then(|v| v.as_str()).ok_or_else(|| anyhow!("[{operation_id}] script_asset requires name"))?.to_string();
            let description = resource.get("description").and_then(|v| v.as_str()).unwrap_or("").to_string();
            let language = resource.get("language").and_then(|v| v.as_str()).unwrap_or("python3").to_string();

            let mut script_entity = json!({"ml_version": "4.0.0", "language": {"name": language}});
            if let Some(sw) = resource.get("software_spec").and_then(|v| v.as_str()) {
                script_entity["runtime"] = json!({"name": sw});
            }
            let create_body = json!({
                "metadata": {"name": name, "description": description, "asset_type": "script", "origin_country": "us"},
                "entity": {"script": script_entity}
            });

            tracing::debug!(
                target: "wxctl::substage::provider",
                operation_id = %operation_id,
                resource_type = "script_asset",
                name = %name,
                language = %language,
            ...
            let resp: Value = client.execute(operation_id, spec).await.map_err(|e| anyhow!("[{operation_id}] script_asset create POST failed: {e}"))?;

            // Upload the file attachment; `resource` is untouched so source_path/name/software_spec are still readable.
            upload_script_asset(resource, &resp, client, operation_id).await?;
            ...
            match find_asset_by_name(resource, name, client, operation_id).await? {
                Some(existing) => {
                    tracing::debug!(
                        ...
                        resource_type = "script_asset",
                        name = %name,
                        "adopt: existing script_asset matched by name"
                    );
                    Ok(Some(existing))
                }
                None => Ok(None),
    ...
    /// downstream refs `${script_asset.x.asset_id}` resolve — the engine stores the
...
async fn upload_script_asset(resource: &Value, response: &Value, client: &HttpClient, operation_id: &str) -> Result<()> {
    let source_path = resource.get("source_path").and_then(|v| v.as_str()).ok_or_else(|| anyhow!("[{operation_id}] script_asset requires source_path"))?;
    let asset_name = resource.get("name").and_then(|v| v.as_str()).unwrap_or("script");
    ...
    let att: Value = client.execute(operation_id, att_spec).await.map_err(|e| anyhow!("[{operation_id}] script_asset attachment create failed (asset_id={asset_id}): {e}"))?;
    ...
    .map_err(|e| anyhow!("[{operation_id}] script_asset file PUT failed (asset_id={asset_id}): {e}"))?;

    if !put_resp.status().is_success() {
        let status = put_resp.status();
        let body = put_resp.text().await.unwrap_or_default();
        return Err(anyhow!("[{operation_id}] script_asset file PUT failed ({status}) for asset_id={asset_id}: {body}"));
    }
    ...
    let _: Value = client.execute(operation_id, complete_spec).await.map_err(|e| anyhow!("[{operation_id}] script_asset attachment complete failed (asset_id={asset_id}): {e}"))?;
    ...
    tracing::debug!(
        ...
        asset_id = %asset_id,
        bytes = bytes_len,
        "script_asset uploaded and finalized"
    );
    Ok(())
}
...
async fn find_asset_by_name(resource: &Value, name: &str, client: &HttpClient, operation_id: &str) -> Result<Option<Value>> {
    ...
    let spec = with_scope(RequestSpec::new(Method::POST, "/v2/asset_types/script/search").body(BodyKind::Json(body)), resource);
