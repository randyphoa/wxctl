...
/// Merge endpoint/status fields surfaced by a post-provisioning GET into the
/// create response, so `location: Computed` fields become referenceable
/// downstream (e.g. `${presto_engine.x.external_host_name}` on an Orchestrate
/// Presto connection). The external endpoint is assigned during provisioning and
/// is absent from the create POST response, so the handler re-fetches once RUNNING
/// and calls this. Only keys present in `fresh` are copied; missing keys and any
/// existing response keys are left untouched.
pub(super) fn merge_computed_fields(response: &mut Value, fresh: &Value, keys: &[&str]) {
    if let (Some(obj), Some(fresh_obj)) = (response.as_object_mut(), fresh.as_object()) {
        for key in keys {
            if let Some(v) = fresh_obj.get(*key) {
...
mod tests {
    ...
    // Present keys are copied; absent keys and unrelated response keys are left
    // as-is; non-requested fresh keys are not copied. This is the merge that
    // makes `${presto_engine.x.external_host_name}` resolvable.
    #[test]
    fn merge_computed_fields_copies_present_keys_only() {
        use serde_json::json;
        let mut response = json!({"id": "presto-1", "display_name": "churn"});
        let fresh = json!({"external_host_name": "https://host:443", "status": "RUNNING", "unrelated": "x"});
        merge_computed_fields(&mut response, &fresh, &["external_host_name", "host_name", "status"]);
        assert_eq!(response["external_host_name"], json!("https://host:443"));
        assert_eq!(response["status"], json!("RUNNING"));
        assert!(response.get("host_name").is_none()); // absent in fresh → not inserted
        assert_eq!(response["display_name"], json!("churn")); // untouched
        assert!(response.get("unrelated").is_none()); // key not in the requested list → not copied
    }
}
