...
impl ResourceHandler for MilvusServiceHandler {
    ...
    fn recover_from_create_error<'a>(&'a self, resource: &'a Value, error: &'a anyhow::Error, client: &'a HttpClient, operation_id: &'a str) -> Pin<Box<dyn Future<Output = Result<Option<Value>>> + Send + 'a>> {
        Box::pin(async move {
            if !engine_lifecycle::is_recoverable_create_error(error) {
                return Ok(None);
            }
            let Some(display_name) = resource.get("display_name").and_then(|v| v.as_str()) else {
                return Ok(None);
            };
            engine_lifecycle::adopt_by_display_name(client, BASE_PATH, "milvus_service", display_name, operation_id).await
        })
    }
}
