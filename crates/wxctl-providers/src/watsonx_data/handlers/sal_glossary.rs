...
impl ResourceHandler for SalGlossaryHandler {
    fn pre_create<'a>(&'a self, resource: &'a mut Value, _fields: &'a [FieldDescriptor], client: &'a HttpClient, _endpoint: &'a str, operation_id: &'a str) -> Pin<Box<dyn Future<Output = Result<HookOutcome>> + Send + 'a>> {
        Box::pin(async move {
            ...
            // Q2 local-hash fallback: a FAILED upload bailed above (no record →
            // next apply re-runs); success records the desired hash so the next
            // apply is NoChange. Best-effort.
            super::sal_enrichment_job::record_local_hash(resource, client, "sal_glossary", operation_id);
    ...
    fn post_validate<'a>(&'a self, resource: &'a mut Value, _operation_id: &'a str) -> Pin<Box<dyn Future<Output = Result<()>> + Send + 'a>> {
        Box::pin(async move {
            // Stamp the CSV *content* hash before the generic identity-hash step
            // (post_validate runs first in the validation pipeline) so identity
            // follows the data, not the machine-dependent path. Missing file →
            // skip the stamp (plan/destroy stay usable; an apply-create still
            // fails in pre_create with the clear file-not-found).
            if let Some(csv) = resource.get("glossary_csv").and_then(|v| v.as_str())
                && Path::new(csv).exists()
                && let Ok(h) = crate::util::hash_file_blake3(Path::new(csv))
            {
                resource["csv_content_hash"] = Value::String(h);
            }
            Ok(())
        })
    }
