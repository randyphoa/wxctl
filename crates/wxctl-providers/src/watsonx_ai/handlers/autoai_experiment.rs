...
impl ResourceHandler for AutoaiExperimentHandler {
    ...
    fn post_discover<'a>(&'a self, remote_data: &'a mut Value, client: &'a HttpClient, operation_id: &'a str, is_apply: bool) -> Pin<Box<dyn Future<Output = Result<()>> + Send + 'a>> {
        ...
        Box::pin(async move {
            ...
            // The trainings GET requires the space/project scope — source it from the
            // discovered run (the create path passes it explicitly).
            let space_id = remote_data.pointer("/entity/space_id").or_else(|| remote_data.pointer("/metadata/space_id")).and_then(|v| v.as_str()).map(|s| s.to_string());
            let terminal = wait_for_training_terminal(client, &training_id, space_id.as_deref(), operation_id).await?;
            if let Some(obj) = remote_data.as_object_mut() {
                normalize_run_into(&terminal, obj);
            }
            Ok(())
