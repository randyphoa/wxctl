...
use crate::cloud_object_storage::cos_client::{CosRequest, parse_s3_error};
...
async fn put_object(resource: &mut Value, client: &HttpClient, operation_id: &str) -> Result<HookOutcome> {
    ...
    // Raw path — CosClient::send percent-encodes it once for both wire and signature.
    let path = format!("/{bucket}/{key}");

    // Drift check: the PUT below stamps `x-amz-meta-wxctl-sha256`; when a HEAD shows
    // the remote object already carries the same content hash, skip the re-upload so
    // repeat applies converge without pushing the full body again. 404 → object absent
    // → proceed to PUT; any other HEAD failure is real (auth, endpoint) and propagates.
    let head = cos.send(CosRequest { region: &region, method: Method::HEAD, path: &path, ..Default::default() }, operation_id).await?;
    match head.status.as_u16() {
        200 if head.header("x-amz-meta-wxctl-sha256") == Some(sha256_hex.as_str()) => {
            let etag = head.header("etag").map(|s| s.trim_matches('"').to_string()).unwrap_or_default();
            tracing::info!(
                ...
                source = %source_label,
                etag = %etag,
                "s3_object content unchanged (wxctl-sha256 match) — skipping PUT"
            );
        ...
        s if (200..300).contains(&s) || s == 404 => {} // hash differs / absent → PUT
        status => {
            let err = parse_s3_error(&head.body_str());
            bail!("[{}] HEAD object {bucket}/{key} failed: HTTP {status} {} — {}", error_codes::H700, err.code, err.message);
        }
    }
...
fn resolve_region(resource: &Value) -> Result<String> {
    ...
    // WXCTL_COS_REGION is the repo-wide convention; COS_REGION kept as a legacy fallback.
    for var in ["WXCTL_COS_REGION", "COS_REGION"] {
