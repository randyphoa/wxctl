...
impl ResourceHandler for S3BucketHandler {
    ...
    fn pre_update<'a>(&'a self, _current: &'a Value, desired: &'a mut Value, _fields: &'a [FieldDescriptor], client: &'a HttpClient, _endpoint: &'a str, operation_id: &'a str) -> Pin<Box<dyn Future<Output = Result<HookOutcome>> + Send + 'a>> {
        Box::pin(async move { update_bucket_tags(desired, client, operation_id).await })
    }
...
async fn update_bucket_tags(resource: &mut Value, client: &HttpClient, operation_id: &str) -> Result<HookOutcome> {
    let name = require_str(resource, "name")?.to_string();
    let region = require_str(resource, "region")?.to_string();
    let storage_class = resource.get("storage_class").and_then(|v| v.as_str()).unwrap_or(DEFAULT_STORAGE_CLASS).to_string();
    let force_destroy = resource.get("force_destroy").and_then(|v| v.as_bool()).unwrap_or(false);
    ...
    let tags = resource.get("tags").and_then(|v| v.as_array()).unwrap_or(&empty_tags);
    put_bucket_tagging(&cos, &region, &name, tags, operation_id).await?;
