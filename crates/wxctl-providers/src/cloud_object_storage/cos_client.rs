...
impl CosClient {
    ...
    pub async fn send(&self, req: CosRequest<'_>, operation_id: &str) -> Result<CosResponse> {
        ...
        let mut url = format!("{endpoint}{}", req.path);
        ...
        match &self.auth {
            CosAuth::Hmac { access_key, secret_key } => {
                let authorization = sigv4_authorization(&req.method, req.path, &req.query, &signed_headers, &payload_hash, req.region, S3_SERVICE, access_key, secret_key, &amz_date, &date_stamp);
...
pub fn sigv4_authorization(method: &Method, path: &str, query: &BTreeMap<String, String>, headers: &BTreeMap<String, String>, payload_hash: &str, region: &str, service: &str, access_key: &str, secret_key: &str, amz_date: &str, date_stamp: &str) -> String {
    // 1. Canonical request
    let canonical_path = urlencode_path(path);
    ...
    let canonical_request = format!("{}\n{}\n{}\n{}\n{}\n{}", method.as_str(), canonical_path, canonical_query, canonical_headers, signed_headers, payload_hash);
