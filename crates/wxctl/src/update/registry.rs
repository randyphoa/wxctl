...
/// `wxctl/<version> (<os>; <arch>)`, built once into a `&'static str` so it
/// satisfies `add_header`'s client-lifetime bound on the header refs.
fn user_agent() -> &'static str {
    static UA: OnceLock<String> = OnceLock::new();
    UA.get_or_init(|| format!("wxctl/{} ({}; {})", env!("CARGO_PKG_VERSION"), std::env::consts::OS, std::env::consts::ARCH)).as_str()
}
