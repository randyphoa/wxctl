...
async fn main() {
    ...
    if let Err(e) = result {
        ...
        if !output::styled_error_rendered() {
            // Route through the theme (honors NO_COLOR / WXCTL_COLOR / non-TTY → no ANSI)
            // and send the error to stderr, not stdout.
            let theme = output::color::Theme::resolve(None);
            eprintln!("\n{}", theme.paint(output::color::Color::Red, &format!("Error: {:#}", e)));
        }
    ...
    if let Some(notice) = update_check.join_timeout() {
        let theme = output::color::Theme::resolve(None);
        for line in output::notice::render_notice(&theme, &notice) {
            eprintln!("{line}");
        }
