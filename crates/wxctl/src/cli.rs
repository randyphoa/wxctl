...
Examples:
  ...
  wxctl init --template        Print a profile template to fill in manually"
    ...
    Init {
        /// Config file(s) to scan for required services. If omitted, configures all services.
        #[arg(short = 'f', long = "filename", value_name = "FILE")]
        config: Vec<String>,
        /// Print a profile template with placeholders instead of prompting interactively.
        #[arg(long)]
        template: bool,
    },
...
pub enum McpCommands {
    ...
    Serve {
        /// Expose only the 4 read-only tools (list/explain/validate/plan); do not register apply/destroy/test. Recommended for unattended agents — mutation is impossible by construction.
        #[arg(long)]
        read_only: bool,
    },
}
