...
pub struct Cli {
    ...
    /// Progress rendering for apply/destroy/plan/test: auto (animate on a TTY,
    /// stream plain otherwise), tty (force the live region), plain (stream
    /// append-only lines, keeps scrollback usable), none (suppress progress).
    /// The panel draws to stderr; stdout stays clean for `--output json`.
    /// Also settable via `WXCTL_PROGRESS`.
    #[arg(long, value_enum, value_name = "MODE", global = true)]
    pub progress: Option<crate::output::progress::ProgressMode>,
...
Examples:
  ...
  wxctl validate -f config.yaml --output json                Output structured JSON errors"
    ...
    Validate {
        ...
        /// Deployment flavor for bridge advisories (saas|software). Omit for the
        /// conservative default (only bridges active on every deployment).
        #[arg(long, value_enum)]
        deployment: Option<ComposeDeployment>,
    },
