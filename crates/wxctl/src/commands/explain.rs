use crate::commands::resources::{deploy_tags, product_name};
...
use crate::output::resource_format::{ResourceFormat, render};
use anyhow::Result;
use wxctl_providers::dependency_graph::deployment_support;
use wxctl_schema::explain::type_label;
use wxctl_schema::explain::{ExplainField, ExplainView};

pub fn execute(kind: &str, format: ResourceFormat) -> Result<()> {
...
fn render_table(view: &ExplainView) {
    ...
    println!("    {} {}", panel.paint(Role::Meta, "envelope  "), panel.paint(Role::Meta, view.authoring.envelope));
    println!("    {} {}", panel.paint(Role::Meta, "ref_name  "), panel.paint(Role::Meta, view.authoring.ref_name));
    println!("    {} {}", panel.paint(Role::Meta, "reference "), panel.paint(Role::Meta, view.authoring.reference_syntax));
    println!();
}
