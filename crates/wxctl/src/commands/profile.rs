...
fn check_credentials(theme: &Theme, service: &str, auth_type: &str, svc_val: &serde_json::Value) -> String {
    let has_field = |field: &str| -> bool { svc_val.get(field).and_then(|v| v.as_str()).map(|s| !s.is_empty()).unwrap_or(false) };
...
async fn validate(name: Option<&str>, cli_profile: Option<&str>, profile_path: Option<&str>, no_connect: bool) -> Result<()> {
    ...
    for (svc_name, svc_val) in &sorted {
        ...
        if auth_type != "none" && !auth_type.is_empty() {
            let has_config_field = |field: &str| -> bool { svc_val.get(field).and_then(|v| v.as_str()).map(|s| !s.is_empty()).unwrap_or(false) };
            let has_creds = match auth_type {
                "apikey" => has_config_field("apikey") || std::env::var(format!("{}_APIKEY", svc_name.to_uppercase())).is_ok(),
        ...
        if !no_connect && !url.is_empty() && url::Url::parse(url).is_ok() {
    ...
    if errors.is_empty() && warnings.is_empty() {
        println!("{}", theme.paint(Color::Green, &format!("Profile '{}' is valid ({} services configured)", target, services.len())));
    } else {
...
async fn check_service(url: &str, auth_type: &str, svc_val: &serde_json::Value) -> ServiceStatus {
    ...
    let get_field = |field: &str| -> Option<String> { svc_val.get(field).and_then(|v| v.as_str()).filter(|s| !s.is_empty()).map(|s| s.to_string()) };
