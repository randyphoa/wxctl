...
fn execute_pass4(scaffold_dir: &str, input: Option<&str>, config: Option<&str>, output_path: Option<&str>) -> Result<()> {
    ...
    let original_input = match input {
        ...
        None => read_cwd_use_case(),
    };
    if original_input.trim().is_empty() {
        eprintln!("warning: no use-case context (pass --input or place use-case.txt in the working directory); implementation prompt assembled without it");
    }
...
/// Fallback use-case sentence when `--input` is absent: prefer `use-case.txt`, then legacy `input.txt`, in cwd.
fn read_cwd_use_case() -> String {
    std::fs::read_to_string("use-case.txt").or_else(|_| std::fs::read_to_string("input.txt")).unwrap_or_default()
}
