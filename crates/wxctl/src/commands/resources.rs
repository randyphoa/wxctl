...
/// Render pre-built rows in a text format: the grouped panel view for `Table`
/// (and any non-Markdown text fallback), a GitHub-flavored table for `Markdown`.
/// JSON / YAML are handled by the `render` dispatcher over the serializable rows.
