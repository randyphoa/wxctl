...
fn apply_success_screen(p: &Panel) -> String {
    ...
    lines.extend(render_exec_footer(
        p,
        &ExecFooter { outcome: ExecOutcome::Ok, command: "apply".into(), created: 3, updated: 0, deleted: 0, failed: 0, duration_ms: 4770, urls: vec![CreatedUrl { name: "agent.httpbin_agent".into(), url: "https://api.watson-orchestrate.ibm.com/agents/abc123".into() }], run_id: "RUNID".into() },
    ));
    lines.join("\n")
}
...
fn apply_failure_screen(p: &Panel) -> String {
    ...
    lines.extend(render_exec_footer(p, &ExecFooter { outcome: ExecOutcome::Failed, command: "apply".into(), created: 1, updated: 0, deleted: 0, failed: 1, duration_ms: 2400, urls: vec![], run_id: "20260612-000000-apply-def456".into() }));
    lines.join("\n")
}
...
fn destroy_success_screen(p: &Panel) -> String {
    ...
    lines.extend(render_exec_footer(p, &ExecFooter { outcome: ExecOutcome::Ok, command: "destroy".into(), created: 0, updated: 0, deleted: 3, failed: 0, duration_ms: 1080, urls: vec![], run_id: "RUNID".into() }));
    lines.join("\n")
}
...
fn test_screen(p: &Panel) -> String {
    ...
    lines.extend(render_exec_footer(p, &ExecFooter { outcome: ExecOutcome::Failed, command: "test".into(), created: 1, updated: 0, deleted: 0, failed: 1, duration_ms: 5200, urls: vec![], run_id: "20260612-000000-test-aaa111".into() }));
    lines.join("\n")
}
...
fn apply_success_screen_invariants() {
    ...
    // Option A: each created row shows its backend resource id.
    assert!(out.contains("[id=805591f1-148f-4f79-8c95-be4785cd0733]"), "tool row shows its create id: {out}");
    assert!(out.contains("[id=5d59f99e-95ae-4b54-8719-3cb993fa0072]"), "agent row shows its create id: {out}");
