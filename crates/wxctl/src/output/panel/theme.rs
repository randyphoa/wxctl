...
/// Unicode-capability decision shared by color-mode and glyph-set resolution so
/// the two never disagree (spec AC9). Plain mode is always ascii (the pure-ASCII
/// guarantee). A non-plain mode implies unicode-capable — an explicit
/// `WXCTL_COLOR=dark` forces color even when stdout is piped, so the glyphs must
/// follow rather than fall back to ascii. The Windows `vt_enabled()` floor still
/// forces ascii when VT can't be enabled, so raw unicode never reaches a legacy
/// conhost; on Unix the (piped-sensitive) `vt_enabled()` probe is not consulted.
/// `WXCTL_GLYPHS` remains the explicit override (honored inside `GlyphSet::resolve`).
pub fn unicode_capable(theme: &Theme) -> bool {
    !theme.is_plain() && (cfg!(not(windows)) || vt_enabled())
}
