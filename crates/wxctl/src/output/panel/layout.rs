...
use crate::output::panel::theme::{Color, Role, Theme, paint_role, unicode_capable};
...
impl Panel {
    ...
    /// Resolve a `Panel` from the environment/terminal, deriving color mode,
    /// width, and glyph set from ONE aligned capability decision
    /// (`unicode_capable`) so color and glyphs never disagree. Use in the
    /// `resources`/`explain`/`runs` entry points and the top-level error renderer.
    pub fn resolve(config_preference: Option<&str>) -> Self {
        let theme = Theme::resolve(config_preference);
        let glyphs = GlyphSet::resolve(unicode_capable(&theme));
        Self::new(theme, Self::resolve_width(), glyphs)
    }
