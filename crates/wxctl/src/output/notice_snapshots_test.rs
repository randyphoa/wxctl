...
fn render(mode: ColorMode) -> String {
    render_notice(&Theme::new(mode), &full_notice(), false).join("\n")
}
...
/// Batch B coverage: under an npm install the upgrade line points at
/// `npm update -g wxctl`, not the `wxctl update` self-updater.
#[test]
fn npm_upgrade_line_uses_npm_command() {
    let out = render_notice(&Theme::new(ColorMode::Plain), &full_notice(), true).join("\n");
    assert!(out.contains("Run `npm update -g wxctl` to upgrade"), "npm upgrade command present: {out}");
    assert!(!out.contains("Run `wxctl update` to upgrade"), "no self-update command for npm: {out}");
}
...
fn render_changelog(mode: ColorMode) -> String {
    render_notice(&Theme::new(mode), &full_notice_with_changelog(), false).join("\n")
}
