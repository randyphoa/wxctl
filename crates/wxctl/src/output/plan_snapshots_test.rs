...
fn success_screen(p: &Panel) -> String {
    ...
    lines.extend(render_footer(p, &Footer { outcome: Outcome::PlanOk, command: "plan".into(), created: 2, updated: 1, deleted: 0, retained: 0, skipped: 0, undetermined: 0, duration_ms: 975, run_id: "RUNID".into(), config_hint: "examples/declarative/simple/config.yaml".into() }));
    lines.join("\n")
}
...
fn no_changes_screen(p: &Panel) -> String {
    ...
    lines.extend(render_footer(p, &Footer { outcome: Outcome::PlanNoChanges, command: "plan".into(), created: 0, updated: 0, deleted: 0, retained: 0, skipped: 0, undetermined: 0, duration_ms: 738, run_id: "RUNID".into(), config_hint: "examples/declarative/wml/config.yaml".into() }));
    lines.join("\n")
}
