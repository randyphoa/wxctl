...
use crate::output::formatters::{format_dependency_event, format_error_stream_line};
...
use crate::output::panel::theme::vt_enabled;
use crate::output::panel_render::{ExecWidths, exec_marker_action_label, render_changes, render_errors, render_exec_footer, render_execution, render_footer};
use crate::output::sections::{ChangeMarker, ChangeRow, ChangesSection, CreatedUrl, ErrorBlock, ErrorsSection, ExecFooter, ExecMarker, ExecOutcome, ExecRow, ExecutionSection, Footer, Outcome, PipelineRow, PipelineSection};
...
pub struct OutputCollector {
    ...
    dependencies: Vec<DependencyEvent>,
...
/// Reserved width of the `[id=…]` annotation in the live Name column, sized to a 36-char
/// UUID: `id_annotation` renders `"  [id=<id>]"` = 2 spaces + `[id=` (4) + id + `]` (1).
const ID_SLOT_RESERVE: usize = 2 + 4 + 36 + 1;
...
impl OutputCollector {
    pub fn new(operation_id: String, theme: Theme) -> Self {
        let multi = if !theme.is_plain() { Some(MultiProgress::with_draw_target(indicatif::ProgressDrawTarget::stdout())) } else { None };
    ...
    /// before any row has run. Sizes the Name column to `name + a reserved [id=…] slot` for
    /// id-returning operations (everything but deletes) so the columns don't shift when the
    /// first backend id arrives.
    fn compute_exec_widths(&self) -> ExecWidths {
        ...
        for d in &self.decisions {
            ...
            let marker = decision_to_exec_marker(&d.decision);
            action_w = action_w.max(exec_marker_action_label(marker).chars().count());
            let id_reserve = if marker == ExecMarker::Deleted { 0 } else { ID_SLOT_RESERVE };
            name_w = name_w.max(d.resource_name.chars().count() + id_reserve);
        }
        ExecWidths::new(kind_w, name_w, action_w)
    }
    ...
    fn panel(&self) -> Panel {
        ...
        let glyphs = GlyphSet::resolve(vt_enabled() && !self.theme.is_plain());
        Panel::new(self.theme.clone(), width, glyphs)
    }
    ...
    /// Add dependency event (suppressed in new format)
    pub fn add_dependency(&mut self, event: DependencyEvent) {
        let formatted = format_dependency_event(&self.theme, &event);
    ...
    pub fn add_error(&mut self, event: ErrorEvent) {
        ...
        self.emit(&format_error_stream_line(&self.theme, &event));
    ...
    pub fn print_header(&self, command: &str, resource_count: usize, profile: Option<&str>, config_paths: &[String]) {
        let sep = self.theme.paint(Color::Dim, " \u{2022} ");
    ...
    fn build_footer(&self, command: &str, has_errors: bool) -> Footer {
        let outcome = if has_errors || self.summary.failed > 0 {
            Outcome::Failed
    ...
    fn build_exec_footer(&self, command: &str, has_errors: bool) -> ExecFooter {
        let outcome = if has_errors || self.summary.failed > 0 { ExecOutcome::Failed } else { ExecOutcome::Ok };
        let urls: Vec<CreatedUrl> = self.summary.resource_urls.iter().map(|(name, url)| CreatedUrl { name: name.clone(), url: url.clone() }).collect();
        ExecFooter { outcome, command: command.to_string(), created: self.summary.created, updated: self.summary.updated, deleted: self.summary.deleted, failed: self.summary.failed, duration_ms: self.summary.total_duration_ms, urls, run_id: self.run_id.clone() }
    }
    ...
    pub fn record_test_complete(&mut self, test_name: &str, passed: bool, completed: usize, total: usize) -> ClearSpinnerPlan {
        ...
        if let Some(ref stage_pb) = self.stage_spinner {
            let status = if passed { self.theme.paint(Color::Green, "✓") } else { self.theme.paint(Color::Red, "✗") };
...
fn error_event_to_block(e: &ErrorEvent) -> ErrorBlock {
    ErrorBlock { stage: e.stage.clone(), code: e.error_code.clone(), kind: e.resource_type.clone(), name: e.resource_name.clone(), field_path: e.field_path.clone(), message: e.message.clone(), fix: e.fix.clone() }
}
...
/// `println!`. Shared by all `*RenderPlan::execute()` paths.
...
impl StartSpinnerPlan {
    ...
    pub fn execute(self) -> (String, Option<ProgressBar>, usize) {
        ...
        let row_id = if let Some(rows) = self.rows_handle {
            ...
            let id = guard.len();
            guard.push(AnimatorRow { pb: pb.clone(), effect, last_msg: None });
            id
        } else {
            0
        };
    ...
    pub fn execute(self) -> Vec<(String, ProgressBar, usize)> {
        ...
        for r in self.rows {
            ...
            let row_id = {
                ...
                let id = guard.len();
                guard.push(AnimatorRow { pb: pb.clone(), effect: Effect::ExecRowPending { panel: self.panel.clone(), widths: self.widths.clone(), kind: r.kind, name: r.name, last: r.last }, last_msg: None });
                id
            };
