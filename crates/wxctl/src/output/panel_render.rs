...
use crate::output::sections::{AdvisoriesSection, ChangeMarker, ChangesSection, CreatedUrl, ErrorBlock, ErrorsSection, ExecFooter, ExecMarker, ExecOutcome, ExecRow, ExecutionSection, Footer, Outcome, PipelineSection};
...
/// `▌ Advisories` section: warn-level, non-blocking suggestions (Phase 3 V505 bridge
/// advisory). Mirrors `render_errors` with a caution marker instead of the danger
/// cross. Renders nothing when empty.
pub fn render_advisories(panel: &Panel, section: &AdvisoriesSection) -> Vec<String> {
    if section.blocks.is_empty() {
        return Vec::new();
    }
    let mut out = vec![panel.section("Advisories", None)];
    for b in &section.blocks {
        out.push(String::new());
        out.push(format!("    {} {}  {}", panel.paint(Role::Caution, panel.g("bang")), panel.paint(Role::Heading, &b.resource), panel.paint(Role::Meta, &b.code)));
        for line in panel.wrap_hanging(&b.message, 6) {
            out.push(line);
        }
        out.push(format!("      {} {}", panel.paint(Role::Active, "fix"), b.suggestion));
    }
    out
}

/// Generic remediation for a top-level error with no parseable `WXCTL-<code>`.
const GENERIC_TOP_LEVEL_FIX: &str = "re-run with --full-trace for the full error chain (wxctl debug)";

/// Render an early / codeless failure (config-load, missing profile) through the
/// same `▌ Errors` panel idiom as in-pipeline errors, so both surfaces are
/// identical. Parses a leading `WXCTL-<code>:` prefix when present (config-load
/// errors carry it, e.g. `WXCTL-V301:`) and strips it from the shown message;
/// derives the fix via `fix_for`, falling back to a generic line for codeless
/// errors. Always returns exactly one rendered block (never empty in practice);
/// the caller treats an empty `Vec` as "rendering yielded nothing" and falls
/// back to a bare line.
pub fn render_top_level_error(panel: &Panel, message: &str) -> Vec<String> {
    let (code, shown) = parse_wxctl_code(message);
    let fix = code.as_deref().and_then(|c| crate::output::remediation::fix_for(c, None)).unwrap_or_else(|| GENERIC_TOP_LEVEL_FIX.to_string());
    let block = ErrorBlock { stage: "command".into(), code: code.unwrap_or_default(), kind: None, name: None, field_path: None, message: shown, fix };
    render_errors(panel, &ErrorsSection { blocks: vec![block] })
}

/// Split a leading `WXCTL-<code>:` prefix off `message`. Returns `(Some(code),
/// remainder)` when the prefix is present, else `(None, message)`. The remainder
/// is trimmed of the leading space so the code is not shown twice (head line +
/// message body).
fn parse_wxctl_code(message: &str) -> (Option<String>, String) {
    if let Some(stripped) = message.strip_prefix("WXCTL-")
        && let Some(end) = stripped.find(':').filter(|&i| i > 0)
    {
        let code = format!("WXCTL-{}", &stripped[..end]);
        return (Some(code), stripped[end + 1..].trim_start().to_string());
    }
