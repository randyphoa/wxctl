...
/// sees stage / substage / decision / error events regardless of the operator's
/// RUST_LOG.
pub const COLLECTOR_FILTER: &str = "wxctl::stage=trace,wxctl::substage=trace,wxctl::decision=trace,wxctl::error=trace,wxctl::summary=trace";
...
/// Layer that forwards stage / substage / decision / error events
    ...
    S: Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: Context<'_, S>) {
        ...
        if !(target.starts_with("wxctl::decision") || target.starts_with("wxctl::error") || target.starts_with("wxctl::summary")) {
            return;
        }
