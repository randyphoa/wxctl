...
pub enum Color {
    ...
    Dim,
    BoldWhite,
    // The ANSI reset primitive (`\x1b[0m`). Still handled by the ANSI renderers,
    // but no longer produced in the binary since `explain`'s verb coloring went
    // neutral (Phase 2 deleted `verb_color`, its only non-test constructor).
    // Kept as a complete-the-enum primitive; `allow(dead_code)` silences the
    // now-no-producer warning under `clippy -D warnings`.
    #[allow(dead_code)]
    Reset,
}
...
impl Theme {
    ...
    /// Resolve the theme for **stdout**-bound output (tables from `resources` /
    /// `explain`, machine documents). Gates auto-detection on stdout's TTY.
    ///
    /// Priority:
    /// 1. NO_COLOR env var (any value) -> Plain
    /// 2. WXCTL_COLOR env var -> auto|always|never|dark|light
    /// 3. config_preference from ~/.wxctl/profiles.yaml preferences.color_theme
    /// 4. Auto-detect (dark default)
    pub fn resolve(config_preference: Option<&str>) -> Self {
        Self::resolve_for(config_preference, std::io::stdout().is_terminal())
    }

    /// Resolve the theme for **stderr**-bound output (the progress panel). Same
    /// precedence as [`resolve`], but auto-detection gates on stderr's TTY —
    /// so `wxctl apply > file` keeps a colored panel on the terminal while the
    /// redirected stdout stays clean.
    pub fn resolve_for_stderr(config_preference: Option<&str>) -> Self {
        Self::resolve_for(config_preference, std::io::stderr().is_terminal())
    }

    /// Shared resolution over an explicit "is the target stream a TTY?" bit.
    fn resolve_for(config_preference: Option<&str>, is_tty: bool) -> Self {
        ...
        if let Ok(val) = std::env::var("WXCTL_COLOR") {
            return match val.to_lowercase().as_str() {
                ...
                "auto" => Self::auto_detect(config_preference, is_tty),
                _ => Self::auto_detect(config_preference, is_tty),
            };
        }

        // 3 + 4. Auto-detect with config preference fallback
        Self::auto_detect(config_preference, is_tty)
    }

    /// Auto-detect: plain if the target stream is not a TTY, otherwise honor an
    /// explicit config preference, else probe the terminal background via OSC 11.
    ///
    /// The non-TTY gate runs *before* any probe, so piped/CI output resolves to
    /// `Plain` with zero ANSI and zero OSC bytes written.
    fn auto_detect(config_preference: Option<&str>, is_tty: bool) -> Self {
        if !is_tty {
            return Self::new(ColorMode::Plain);
        }
