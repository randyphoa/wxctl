...
pub fn render_notice(theme: &Theme, notice: &UpdateNotice, via_npm: bool) -> Vec<String> {
    ...
    if let Some(latest) = &notice.new_version {
        ...
        let upgrade_cmd = if via_npm { "npm update -g wxctl" } else { "wxctl update" };
        lines.push(paint_role(theme, Role::Meta, &format!("  Run `{upgrade_cmd}` to upgrade")));
    }
