...
fn wxctl(home: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_wxctl")).args(args).arg("--no-update-check").env("HOME", home).env("WXCTL_UPDATE_CACHE_DIR", home.join(".wxctl")).env_remove("CI").env_remove("GITHUB_ACTIONS").env_remove("VISUAL").env_remove("EDITOR").stdin(Stdio::null()).output().expect("spawn wxctl")
}
...
const EXPECTED_SERVICES: &[&str] = &["cloud_object_storage", "common_core", "concert", "concert_workflows", "factsheets", "instana", "openscale", "planning_analytics", "watsonx_ai", "watsonx_data", "watsonx_orchestrate"];
