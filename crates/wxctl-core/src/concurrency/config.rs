...
impl Default for ConcurrencyConfig {
    fn default() -> Self {
        Self { global_limit: default_global_limit(), service_limits: HashMap::from([("wkc".to_string(), 10), ("catalog".to_string(), 20), ("governance".to_string(), 10)]), default_timeout_secs: default_operation_timeout(), request_timeout_secs: default_request_timeout() }
    }
}
...
mod tests {
    ...
    #[test]
    fn default_global_limit_is_50() {
        let config = ConcurrencyConfig::default();
        assert_eq!(config.global_limit, 50);
    }
