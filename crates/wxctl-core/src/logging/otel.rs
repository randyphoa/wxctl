...
/// Returns `None` if no OTel env vars are set, or if `WXCTL_OTEL_FILE` names a
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    let provider = if let Ok(endpoint) = std::env::var("OTEL_EXPORTER_OTLP_ENDPOINT") {
        let exporter = opentelemetry_otlp::SpanExporter::builder().with_tonic().with_endpoint(endpoint).build().expect("Failed to build OTLP span exporter");
