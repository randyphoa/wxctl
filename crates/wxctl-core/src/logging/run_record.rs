...
pub fn prune_runs(root: &Path) {
    ...
    for dir in dirs.into_iter().take(remove) {
...
///
/// `events.jsonl` is appended line-by-line as events arrive; the manifest is
/// held in memory and flushed on `finalize`. All IO is best-effort: a failure
/// sets `record_incomplete` and emits one stderr warning, never an error.
pub struct RunSink {
    dir: PathBuf,
...
impl RunSink {
    ...
    pub fn new(manifest: RunManifest) -> Option<Self> {
        ...
        let events_file = OpenOptions::new().create(true).write(true).truncate(true).open(dir.join("events.jsonl")).ok();
    ...
    /// Set outcome + finished timestamp and flush `manifest.json`. Idempotent.
