...
fn reject_duplicate_service_keys(content: &str, is_json: bool, name: &str) -> Result<()> {
    let parsed: ProfileKeyConfig = if is_json { serde_json::from_str(content).context("Failed to parse JSON config")? } else { serde_norway::from_str(content).context("Failed to parse YAML config")? };
...
fn load_profile(name: &str, profile_path: Option<&str>) -> Result<Profile> {
    let config_path = if let Some(path) = profile_path {
        ...
        let config_dir = home_dir.join(".wxctl");

        // Try config.json first, then config.yaml
        if config_dir.join("config.json").exists() { config_dir.join("config.json") } else { config_dir.join("config.yaml") }
    };
    ...
    reject_duplicate_service_keys(&content, is_json, name)?;

    let config: serde_json::Value = if is_json { serde_json::from_str(&content).context("Failed to parse JSON config")? } else { serde_norway::from_str(&content).context("Failed to parse YAML config")? };
...
///
/// Looks in the given profile_path, or falls back to ~/.wxctl/config.json.
...
pub fn load_color_preference(profile_path: Option<&str>) -> Option<String> {
    ...
    let config: serde_json::Value = if config_path.extension().and_then(|s| s.to_str()) == Some("json") { serde_json::from_str(&content).ok()? } else { serde_norway::from_str(&content).ok()? };
...
fn get_auth_token_with(config: &ServiceConfig, service: &str, auth_type: &str) -> Result<String> {
    match auth_type {
        ...
        "bearer" => {
            let token = config.apikey.as_ref().ok_or_else(|| anyhow::anyhow!("No 'apikey' (bearer token) configured for service '{}'.", service))?;
            Ok(token.clone())
        }
        "hmac" => {
