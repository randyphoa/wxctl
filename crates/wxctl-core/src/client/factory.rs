...
impl ClientFactory {
    ...
    /// The profile's declared top-level `deployment`, parsed. `Ok(None)` when the
    /// profile omits it, distinct from `deployment_for_service`, which collapses an
    /// absent deployment to `Saas`. This `None` is the conservative-default signal for
    /// the validate-surface bridge advisory. `Err` only when a present string is
    /// unparseable. No network.
    pub fn profile_deployment(&self) -> Result<Option<Deployment>> {
        match self.profile.deployment.as_deref() {
            None => Ok(None),
            Some(s) => Deployment::from_str(s).map(Some).with_context(|| format!("invalid deployment in profile '{}'", self.profile_name)),
        }
    }
    ...
    pub fn create_client(&self, service: &str) -> Result<HttpClient> {
        ...
        HttpClient::with_token_manager(url, service.to_string(), auth_type, self.capacity.clone(), token_manager, self.request_timeout_secs, service_config.instance_id.clone(), service_config.namespace.clone(), path_prefix, deployment.clone())
    }
...
/// The wxctl config directory: `WXCTL_CONFIG_DIR` if set and non-empty, else `~/.wxctl`.
///
...
/// records). Returns `None` only when the override is unset and no home dir is
/// discoverable.
pub fn wxctl_config_dir() -> Option<std::path::PathBuf> {
    std::env::var_os("WXCTL_CONFIG_DIR").filter(|v| !v.is_empty()).map(std::path::PathBuf::from).or_else(|| dirs::home_dir().map(|h| h.join(".wxctl")))
}
...
fn get_auth_token_with(config: &ServiceConfig, service: &str, auth_type: &str) -> Result<String> {
    match auth_type {
        ...
        "vault_token" => {
            // HashiCorp Vault static token (typically ${env:VAULT_TOKEN}), carried in `apikey`.
            let token = config.apikey.as_ref().ok_or_else(|| anyhow::anyhow!("No 'apikey' (Vault token) configured for service '{}' (auth_type: vault_token). Set it to a token value, e.g. ${{env:VAULT_TOKEN}}.", service))?;
            Ok(token.clone())
        }
