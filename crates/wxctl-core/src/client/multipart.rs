...
impl HttpClient {
    ...
    async fn request_multipart_internal<'a, T: DeserializeOwned + Send + 'a>(&'a self, operation_id: &'a str, method: Method, path: &'a str, form_data: HashMap<String, Value>, files: Vec<&'a Path>, file_field_name: &'a str) -> Result<T> {
        ...
        // Redacted copy of the form fields for logging — multipart bodies must never
        // reach a log sink raw (same contract as execute_internal's redacted_req_body).
        let redacted_form = crate::logging::redact_sensitive(&Value::Object(form_data.clone().into_iter().collect()));
            ...
            async {
                ...
                if status.is_success() {
                    ...
                    if let Ok(ref rv) = response_value {
                        let redacted_resp = crate::logging::redact_for_log(rv, &[]);
                        crate::log_http_request!(operation_id, &request_id, method.as_str(), &url, status.as_u16(), &redacted_form, &redacted_resp);
                    }
                ...
                let redacted_error = crate::logging::redact_for_log(&serde_json::from_str::<Value>(&error_body).unwrap_or_else(|_| Value::String(error_body.clone())), &[]);
                crate::log_http_request!(operation_id, &request_id, method.as_str(), &url, status.as_u16(), &redacted_form, &redacted_error);
