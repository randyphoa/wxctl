...
pub struct HttpClient {
    ...
    /// HashiCorp Vault namespace (HCP/Enterprise multi-tenancy). When present, sent
    /// as `X-Vault-Namespace` on every request. Non-secret. OSS Vault has no
    /// namespaces — this stays `None` there.
...
impl HttpClient {
    pub fn new(base_url: String, service: String, auth_token: String, auth_type: String, capacity: Arc<CapacityManager>, request_timeout_secs: u64) -> Result<Self> {
        ...
        Self::with_token_manager(base_url, service, auth_type, capacity, token_manager, request_timeout_secs, None, None, String::new(), crate::types::Deployment::Saas)
    }
    ...
    pub fn with_token_manager(
        ...
        path_prefix: String,
        deployment: crate::types::Deployment,
    ) -> Result<Self> {
        ...
        Ok(Self { client, token_manager, base_url, service, auth_type, max_retries: 3, capacity, instance_id, namespace, path_prefix, deployment })
    }
    ...
    pub(crate) fn apply_auth(&self, req: reqwest::RequestBuilder, token: &str) -> Result<reqwest::RequestBuilder, HttpError> {
        match self.auth_type.as_str() {
            ...
            // HashiCorp Vault authenticates via X-Vault-Token; no Authorization header.
            "vault_token" => Ok(req.header("X-Vault-Token", token)),
    ...
    /// Emit the HashiCorp Vault `X-Vault-Namespace` header when this client is
    /// configured with a namespace (HCP/Enterprise multi-tenancy). A no-op for
    /// OSS Vault and every non-vault service, where `namespace` is `None`.
    pub(crate) fn apply_namespace_header(&self, req: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        match &self.namespace {
            Some(namespace) => req.header("X-Vault-Namespace", namespace),
            None => req,
        }
    }
    ...
    async fn execute_internal<'a, T: DeserializeOwned + Send + 'a>(&'a self, operation_id: &'a str, spec: super::request::RequestSpec) -> Result<T> {
        ...
        retry::with_retry(self.max_retries, async |attempt| {
            ...
            async {
                ...
                req = self.apply_namespace_header(req);
...
mod list_envelope_tests {
    ...
    fn every_envelope_key_deserializes() {
        for &key in ENVELOPE_KEYS {
            // Vault's `keys` envelope is nested under `data`; every other key is flat.
            let payload = if key == "keys" { json!({ "data": { "keys": [] } }) } else { json!({ key: [] }) };
...
mod zenapikey_http_tests {
    ...
    fn make_vault_client(namespace: Option<String>) -> HttpClient {
        let cfg = ConcurrencyConfig::default();
        let capacity = Arc::new(CapacityManager::new(&cfg));
        let tm = Arc::new(TokenManager::with_base_url("tok".to_string(), "vault_token".to_string(), "https://vault.example".to_string()));
        HttpClient::with_token_manager("https://vault.example".to_string(), "vault".to_string(), "vault_token".to_string(), capacity, tm, 30, None, namespace, String::new(), crate::types::Deployment::Saas).expect("client construct")
    }

    fn namespace_header(client: &HttpClient) -> Option<String> {
        let req = client.client.get("https://vault.example/x");
        let req = client.apply_namespace_header(req);
        let built = req.build().expect("build request");
        built.headers().get("X-Vault-Namespace").map(|v| v.to_str().expect("ascii header").to_string())
    }

    #[test]
    fn vault_namespace_header_emitted_only_when_set() {
        // Set: X-Vault-Namespace carries the configured namespace verbatim.
        let with_ns = make_vault_client(Some("admin".to_string()));
        assert_eq!(namespace_header(&with_ns).as_deref(), Some("admin"), "namespace set -> header present");
        // Unset (OSS Vault / non-vault service): header omitted entirely.
        let without_ns = make_vault_client(None);
        assert_eq!(namespace_header(&without_ns), None, "namespace unset -> no header");
    }
}
