...
impl TokenManager {
    ...
    async fn refresh_token<'a>(&'a self, client: &'a Client) -> Result<String> {
        match self.auth_type.as_str() {
            ...
            "pa_session" => {
                // Planning Analytics TM1 REST cookie auth. Static-cookie mode returns a
                // pre-acquired paSession value directly; username:password credentials
                // instead run a login round-trip against the gateway (see
                // authenticate_pa_session).
                let token = self.authenticate_pa_session(client).await?;
                Ok(token)
            }
            "vault_token" => {
                // HashiCorp Vault static token: returned verbatim — no exchange, no
                // outbound HTTP (like the zenapikey arm). Long TTL: it is a fixed
                // profile credential, refreshed only when the profile changes.
                let cached = CachedToken::new(self.auth_token.clone(), 365 * 24 * 3600);
                *self.cached_token.lock().await = Some(cached);
                Ok(self.auth_token.clone())
            }
            _ => {
    ...
    async fn authenticate_with_cp4d<'a>(&'a self, client: &'a Client, credentials: &'a str) -> Result<String> {
        ...
        // Parse username:password from credentials — first colon delimits, so
        // passwords may themselves contain ':'.
        let (username, password) = credentials.split_once(':').ok_or_else(|| anyhow::anyhow!("Invalid credentials format for CP4D auth (expected username:password)"))?;
    ...
    /// Acquire a `paSession` cookie value for `auth_type: pa_session`.
    ///
    /// Two modes, distinguished by the credential shape the factory packs:
    /// - **Static cookie** — `auth_token` is a bare, pre-acquired `paSession` value
    ///   (typically `${env:PA_SESSION}`). Cached and returned directly; no round-trip.
    /// - **Login round-trip** — `auth_token` is `username:password`. Posts a
    ///   form-encoded `username`/`password` body to `<base_url>/login`; the gateway
    ///   responds `200` with `Set-Cookie: paSession=<url-encoded-value>; Path=/; HttpOnly`.
    ///   The cookie value is cached and must be echoed back verbatim (URL-encoded,
    ///   undecoded) as `Cookie: paSession=<value>` on subsequent requests.
    ///
    /// PA sessions idle out at ~20 minutes; both modes cache with a matching TTL.
    async fn authenticate_pa_session<'a>(&'a self, client: &'a Client) -> Result<String> {
        if let Some((user, pass)) = self.auth_token.split_once(':')
            && !user.is_empty()
            && !pass.is_empty()
        {
            let base_url = self.base_url.as_ref().ok_or_else(|| anyhow::anyhow!("base_url required for pa_session login round-trip"))?;
            let login_url = format!("{}/login", base_url.trim_end_matches('/'));

            let response = client.post(&login_url).form(&[("username", user), ("password", pass)]).send().await.context("Failed to request Planning Analytics gateway login")?;

            let status = response.status();
            if !status.is_success() {
                let error_body = response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
                return Err(anyhow::anyhow!("Planning Analytics gateway login failed with status {status}: {error_body}; supply a pre-acquired paSession value as the profile 'apikey' (e.g. ${{env:PA_SESSION}}) for static-cookie mode instead"));
            }

            let cookie_value = response.headers().get_all(reqwest::header::SET_COOKIE).iter().find_map(|value| value.to_str().ok().and_then(extract_pa_session));

            return match cookie_value {
                Some(value) => {
                    let cached = CachedToken::new(value.clone(), 20 * 60);
                    *self.cached_token.lock().await = Some(cached);
                    Ok(value)
                }
                None => Err(anyhow::anyhow!("Planning Analytics gateway login succeeded (status {status}) but did not return a paSession cookie; supply a pre-acquired paSession value as the profile 'apikey' (e.g. ${{env:PA_SESSION}}) for static-cookie mode instead")),
            };
        }
        let cached = CachedToken::new(self.auth_token.clone(), 20 * 60);
        *self.cached_token.lock().await = Some(cached);
        Ok(self.auth_token.clone())
    }
}

/// Extract the `paSession` cookie value from a single raw `Set-Cookie` header value, if
/// present. The value is returned verbatim (URL-encoded) — callers must not decode it,
/// since the Planning Analytics gateway expects the same encoded bytes echoed back on the
/// `Cookie` header.
fn extract_pa_session(set_cookie: &str) -> Option<String> {
    let (name, value) = set_cookie.split(';').next()?.trim().split_once('=')?;
    if name.trim() == "paSession" { Some(value.to_string()) } else { None }
}
...
#[cfg(test)]
mod pa_session_token_tests {
    use super::*;
    use reqwest::Client;

    #[tokio::test]
    async fn static_cookie_returns_value_and_makes_no_http_call() {
        // Static-cookie mode: the credential IS the paSession value. Point base_url at a
        // non-routable port with a short timeout — if this arm made an HTTP call it would error.
        let mgr = TokenManager::with_base_url("PASESSION-abc123".to_string(), "pa_session".to_string(), "http://127.0.0.1:1".to_string());
        let client = Client::builder().timeout(std::time::Duration::from_millis(200)).build().expect("client");
        let token = mgr.get_token(&client).await.expect("static-cookie get_token should succeed with no round-trip");
        assert_eq!(token, "PASESSION-abc123");
        assert_eq!(mgr.get_token(&client).await.expect("cached second call"), "PASESSION-abc123");
    }

    #[tokio::test]
    async fn login_mode_posts_to_gateway_and_errors_cleanly_when_unreachable() {
        // A username:password shape selects the login round-trip: a real POST to
        // <base_url>/login. Against a non-routable base_url this fails at the connection
        // stage (not with the old "not yet available" placeholder error).
        let mgr = TokenManager::with_base_url("alice:s3cret".to_string(), "pa_session".to_string(), "http://127.0.0.1:1".to_string());
        let client = Client::builder().timeout(std::time::Duration::from_millis(200)).build().expect("client");
        let err = mgr.get_token(&client).await.expect_err("login round-trip against an unreachable gateway should error");
        assert!(!err.to_string().contains("not yet available"), "should not be the old placeholder error: {err}");
    }

    #[test]
    fn extract_pa_session_parses_the_cookie_value_verbatim() {
        assert_eq!(extract_pa_session("paSession=s%3Aabc.def; Path=/; HttpOnly"), Some("s%3Aabc.def".to_string()));
    }

    #[test]
    fn extract_pa_session_ignores_non_pa_session_cookies() {
        assert_eq!(extract_pa_session("sessionid=xyz; Path=/; HttpOnly"), None);
    }
}
