...
/// Extract tool `ref_name` → `description` mappings from a multi-doc config YAML string.
/// Only `kind: tool` documents with both `ref_name` and `description` are included; any
/// document that fails to parse is skipped. Infallible — the file-reading
/// `tool_descriptions_from_config` and the `wxctl-mcp` `compose_prompt` wrapper both delegate
/// here (single source of the parse loop).
pub fn tool_descriptions_from_yaml(yaml: &str) -> HashMap<String, String> {
...
/// Extract tool `ref_name` → `description` mappings from a multi-doc config YAML file.
/// Reads the file, then delegates to `tool_descriptions_from_yaml`.
pub fn tool_descriptions_from_config(config_path: &str) -> Result<HashMap<String, String>> {
    ...
    Ok(tool_descriptions_from_yaml(&yaml))
}
