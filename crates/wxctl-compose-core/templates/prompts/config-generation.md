...
- **Python tool schemas**: A Python tool's schema lives in `schema.yaml` in its `source_path`, which is authoritative — the apply-time handler loads it and overwrites any inline block. Do NOT put `input_schema` or `output_schema` in the `config.yaml` tool block; author a `schema.yaml` (with `input_schema`, plus `output_schema` when the tool returns structured data) in each Python tool's `source_path`. In that `schema.yaml`, include only **required** input parameters (the LLM passes `null` for optional params, causing 422 errors); if a tool takes no inputs, use `input_schema: {type: object}`. The `config.yaml` tool block carries `source_path` + `binding` only.
