...
# macOS: strip the Gatekeeper quarantine bit. The curl | sh path never sets it,
# but this keeps the binary runnable if it arrived any other way (browser download).
if [ "$plat" = "apple-darwin" ]; then
